...
#### Journal preprint policies 期刊预印本政策
...
Post-publication review is theoretically a great idea, but I’m still waiting to see a system that beats pre-publication review for maintaining scientific quality (even though it may just be an [impossible problem](http://simplystatistics.org/2015/02/09/the-trouble-with-evaluating-anything/)).  
发表后审查在理论上是一个好主意，但我仍在等待看到一个在保持科学质量方面胜过发表前审查的系统（尽管这可能只是一个不可能的问题）。

If you do decide to use post-publication review keep in mind a few things. One is that there are power differentials in science and authoring a review can have implications for your career if it is unnecessarily negative. Another is that people will in general draw some conclusions about you from the way you referee. I would suggest following the refereeing model described above and the [template](https://raw.githubusercontent.com/jtleek/reviews/master/review-template.Rmd) when performing post publication review while keeping your tone measured.  
如果您决定使用发表后审查，请记住以下几点。其一是科学中存在权力差异，撰写一篇评论可能会对你的职业生涯产生影响，如果它是不必要的负面的话。另一个是人们通常会从你的裁判方式中得出一些关于你的结论。我建议在执行发表后审查时遵循上述裁判模型和模板，同时保持审慎的语气。

_Note:_ Parts of this chapter appeared in the Simply Statistics blog post: [“Preprints are great, but post publication peer review isn’t ready for prime time”](http://simplystatistics.org/2016/02/26/preprints-and-pppr/) and the [Leek group guide to reviewing papers](https://github.com/jtleek/reviews).  
注意：本章的部分内容出现在 Simply Statistics 博客文章中：“预印本很棒，但出版后同行评审还没有准备好迎接黄金时段” 和 Leek 小组论文评审指南。
...
The reproducibility debate is over. Data should be made available when papers are published. The [Potti scandal](http://simplystatistics.org/2012/02/27/the-duke-saga-starter-set/) and the [Reinhart/Rogoff scandal](http://simplystatistics.org/2013/04/19/podcast-7-reinhart-rogoff-reproducibility/) have demonstrated the extreme consequences of lack of reproducibility and the reproducibility advocates have taken this one home. The question with reproducibility isn’t “if” anymore it is “how”.  
可重复性的争论结束了。论文发表时应提供数据。 Potti 丑闻和 Reinhart/Rogoff 丑闻证明了缺乏可再现性的极端后果，可再现性的拥护者已经把这一点带回家了。可重复性的问题不再是 “如果”，而是 “如何”。
...
由于两个原因，向再现性的过渡可能很艰难。一是许多生成数据的人缺乏处理和分析数据的培训，即使在基因组学这样的数据饱和领域也是如此。在传统上不被认为是 “数据丰富” 的领域，情况更加严峻。
...
第二个问题是文化和经济问题。它涉及 (1) 我们的进步、资助和晋升制度的激励机制与 (2) 有利于科学和提高可重复性的政策之间的根本脱节。社交媒体上的大部分辩论似乎都将这两个问题混为一谈。我认为值得将辩论分为三个主要支持者：期刊、数据创建者和数据分析师。
...
数据共享，尤其是大型数据集，并不容易，也不便宜。不知道如何共享数据不是借口——要成为一名现代科学家，这是您必须具备的技能之一。

**Post your data to a public repository  
将您的数据发布到公共存储库**

You should post your data to a public repository. You will eventually change websites and managing your own data can be a hassle. Take advantage of field specific repositories or more general data sharing services like [Figshare](http://figshare.com/) to make your data available and citable.  
您应该将数据发布到公共存储库。您最终会更换网站，管理自己的数据可能会很麻烦。利用特定领域的存储库或 Figshare 等更通用的数据共享服务，使您的数据可用和可引用。
...
现在在线发布数据相对简单，但数据通常是完全随意存放的。原因是历史上很少鼓励以深思熟虑或最有用的方式共享数据。为了最大限度地发挥数据的价值和影响，(a) 发布数据的原始版本和整洁版本，(b) 发布与您在 README 中执行的实验相关的元数据，以及 (c) 发布可用于像在论文中那样分析数据。
...
令人惊讶的是，与特定论文相关的数据往往不容易被发现，即使它们是公开的。确保在您的出版物中包含指向您的数据集的直接和永久链接。

### Data sharing - what tools should I use?  
数据共享——我应该使用什么工具？

#### Places you can share data 您可以共享数据的地方

In general you want to post data to a data hosting website. If you post data on your personal website it is almost always impermanent. When you move institutions or change web servers the data will likely disappear as well. [Link rot](https://en.wikipedia.org/wiki/Link_rot) is one of the most fundamental sources of reproducibility in science.  
通常，您希望将数据发布到数据托管网站。如果您在个人网站上发布数据，它几乎总是无常的。当您移动机构或更改网络服务器时，数据也可能会消失。 Link rot 是科学中可重复性最基本的来源之一。

There are a variety of places you can make data publicly available. General purpose sharing sites include the following.  
您可以在很多地方公开数据。通用共享站点包括以下内容。

*   [Figshare](https://figshare.com/) - if you make your data sets publicly available then you can post unlimited data for free. If you want private data sets there are some charges. Figshare accepts all data types and gives you a doi for your data so it can be cited.  
    Figshare - 如果您公开您的数据集，那么您可以免费发布无限数据。如果您想要私有数据集，则需要支付一些费用。 Figshare 接受所有数据类型，并为您的数据提供一个 doi，以便可以引用。
*   [Open Science Framework](https://osf.io/) - can be used to post all types of data sets and also integrates with Figshare.  
    Open Science Framework - 可用于发布所有类型的数据集，也可与 Figshare 集成。
*   [Dataverse](https://dataverse.harvard.edu/) - hosted by Harard, you can again host data for free.  
    Dataverse - 由 Harard 托管，您可以再次免费托管数据。

You can also post data to a variety of [field specific data repositories](http://oad.simmons.edu/oadwiki/Data_repositories). But be careful to ensure that the websites have (a) been around for a while, (b) have a track record of managing data, and (c) make their pricing structure clear if they charge.  
您还可以将数据发布到各种特定领域的数据存储库。但要小心确保网站 (a) 存在一段时间，(b) 有管理数据的记录，以及 (c) 如果收费，请明确其定价结构。

#### Data sharing and manipulation 数据共享和操作

When posting data online, it is helpful to post both the raw and processed version of the data, as well as relevant metadata and instructions. I have posted a [data sharing guide](https://github.com/jtleek/datasharing), some of which is also reproduced in the next section. Karl Broman also has an excellent [data organization guide](http://kbroman.org/dataorg/).  
在线发布数据时，发布数据的原始版本和处理后的版本以及相关的元数据和说明会很有帮助。我已经发布了一个数据共享指南，其中一些也在下一节中转载。 Karl Broman 还有一本出色的数据组织指南。

#### Really big data 真正的大数据

One major unsolved issue with data sharing is if you want to share very large data sets. Many of the most common repositories have file size limits and sharing limits. An alternative is to host them yourself on something like [Dropbox](http://www.dropbox.com/) or [Amazon S3](https://aws.amazon.com/s3/). Unfortunately storing data with these commercial providers (a) makes data more temporary and (b) incurs ongoing costs. Many fields where large data sets are generated have their own custom repositories you can use and that is still the best route if you have a large data set you need to share.  
数据共享的一个主要未解决问题是是否要共享非常大的数据集。许多最常见的存储库都有文件大小限制和共享限制。另一种方法是自己将它们托管在诸如 Dropbox 或 Amazon S3 之类的东西上。不幸的是，与这些商业提供商一起存储数据 (a) 使数据更加临时，并且 (b) 会产生持续的成本。许多生成大型数据集的领域都有自己的自定义存储库，您可以使用它，如果您有大型数据集需要共享，这仍然是最佳途径。
...
最近，由于各种原因，人们对分析对公众保密的数据产生了浓厚的兴趣。这些数据集可能来自像 Facebook 这样的初创公司并且非常有价值，或者它们可能是电子健康记录中受保护的医疗数据并构成隐私风险。科学界出现的一个棘手问题是处理私有数据集。对这些非常有价值的数据的研究可能非常有价值。但它们对科学过程构成了风险。一般来说，如果结果不能被任何人验证，那么一篇论文就不能被认为是科学的。 (a) 没有定义访问论文中使用的数据的过程或 (b) 为外部团体提供验证其结果的机制的论文必须被认为是可疑的，直到通过独立来源进行验证。
...
最后，我认为我们应该更加同情数据生成者的职业问题。我已经编写了方法并提供了代码。然后我看到人们使用我的方法和代码写出非常相似的论文——然后因为产生与我自己的方法非常相似的方法而获得荣誉 / 引用。我一直在密切关注 PLoS One 数据共享新政策的影响。该政策基本上说，如果你发表一篇论文，所有与该论文相关的数据和代码都应在发表时公开，并在他们提交的论文中包含明确的数据共享政策。
...
我认为可重复性辩论已经结束。论文发表时应提供数据。 Potti 丑闻和 Reinhart/Rogoff 丑闻证明了缺乏可再现性的极端后果，可再现性的拥护者已经把这一点带回家了。可重复性的问题不再是 “如果”，而是 “如何”。
...
几乎在任何科学学科中都很难创建一个严肃的、研究质量的数据集。如果您正在研究人类，则需要认真遵守处理人类数据的规则和程序。如果你从事生态学，它可能涉及广泛的实地工作。如果您从事行为研究，可能需要仔细查看数千小时的录像带。
...
一个仔细、严谨和有趣的数据集的价值怎么强调都不过分。在我的领域，Leonid Kruglyak 的小组在仔细的酵母实验中生成测量基因表达和遗传学的数据，在基因组学和统计学中催生了一门全新的学科。
...
问题是生成一个非常好的数据集可能需要几个月甚至几年的时间。绝对有可能在一个非常好的数据集上发表不止一篇论文。但是在数据生成之后，这些论文中的大部分将与数据分析有关，而不是数据生成。如果有十篇论文可以在你的数据集上发表，而你的团队发表了第一篇数据，你可能会发表第二篇或第三篇，但其他人可能会发表 4-10 篇。
...
这可能对科学有利，但对数据生成器的职业生涯不利。询问学术界的任何人，你是更愿意从很棒的论文中获得 6 次引用还是 6 篇很棒的论文，并且 100% 的人都会接受这些论文。
...
我完全同情那些花费大量时间创建数据集并担心他们可能会在以后的论文中被挖到的数据生成器。这是信用文化没有赶上科学文化的地方。如果你写了一个 grant 并生成了一个惊人的数据集，供 50 个不同的人使用——你绝对应该在你的下一个 grant 中获得主要的荣誉。但是，除非您在智力上为下一阶段的分析做出贡献，否则您可能不应该获得作者身份。
...
问题是我们没有比引用权重更高的数据生成器信用中间形式。在短期内，缺乏适当的信用系统可能会导致数据生成者做出以下（完全明智的）决定来关闭他们的数据，然后同时发表多篇论文——就像 ENCODE 所做的那样。这会让每个人都发疯并减缓科学的发展——但在我们的信用系统赶上来之前，它是数据生成器的合适职业选择。

#### Data generators - respect data reuse.  
数据生成器——尊重数据重用。

The editors of the New England Journal of Medicine [posted an editorial](http://www.nejm.org/doi/full/10.1056/NEJMe1516564) showing some moderate level of support for data sharing but also introducing the term “research parasite”:  
《新英格兰医学杂志》的编辑发表了一篇社论，表达了对数据共享的适度支持，但也引入了 “研究寄生虫” 一词：

> A second concern held by some is that a new class of research person will emerge — people who had nothing to do with the design and execution of the study but use another group’s data for their own ends, possibly stealing from the research productivity planned by the data gatherers, or even use the data to try to disprove what the original investigators had posited. There is concern among some front-line researchers that the system will be taken over by what some researchers have characterized as “research parasites.”  
> 一些人担心的第二个问题是，将会出现一类新的研究人员——他们与研究的设计和执行无关，但为了自己的目的而使用另一组的数据，可能会窃取研究人员计划的研究生产力。数据收集者，甚至使用数据来反驳最初调查人员的假设。一些一线研究人员担心该系统将被一些研究人员称为 “研究寄生虫” 的人接管。
...
2.  **”… but use another group’s data for their own ends, possibly stealing from the research productivity planned by the data gatherers, or even use the data to try to disprove what the original investigators had posited.”** The idea that no one should be able to try to disprove ideas with the authors data has been covered extensively. It is worth considering the concern about credit. I think that the traditional way credit has accrued to authors has been citations. But if you get a major study funded, say for 50 million dollars, run that study carefully, sit on a million conference calls, and end up with a single major paper, that could be frustrating. Which is why I think that a better policy would be to have the people who run massive studies get credit in a way that _is not papers_. They should get some kind of formal administrative credit. But then the data should be immediately and publicly available to anyone to publish on. That allows people who run massive studies to get credit and science to proceed normally.  
    “...... 但为了自己的目的而使用另一个小组的数据，可能会窃取数据收集者计划的研究生产力，或者甚至使用这些数据来试图反驳最初调查人员的假设。” 没有人应该能够尝试用作者的数据来反驳想法的想法已被广泛涵盖。值得考虑对信用的担忧。我认为作者获得荣誉的传统方式是引用。但是，如果你获得了一项重大研究的资助，比如说 5000 万美元，仔细地进行这项研究，参加了 100 万次电话会议，最后只发表了一篇主要论文，那可能会令人沮丧。这就是为什么我认为更好的政策是让进行大规模研究的人以非论文的方式获得学分。他们应该获得某种正式的行政信用。但是数据应该立即公开供任何人发布。这使得进行大量研究的人能够获得信贷和科学正常进行。
3.  **“The new investigators arrived on the scene with their own ideas and worked symbiotically, rather than parasitically, with the investigators holding the data, moving the field forward in a way that neither group could have done on its own.”** The story that follows about a group of researchers who collaborated with the NSABP to validate their gene expression signature is very encouraging. But it isn’t the only way science should work. Researchers shouldn’t be constrained to one model or another. Sometimes collaboration is necessary, sometimes it isn’t, but in neither case should we label the researchers “symbiotic” or “parasitic”, terms that have extreme connotations.  
    “新的调查人员带着他们自己的想法来到现场，并与持有数据的调查人员共生而非寄生地工作，以任何一个团队都无法独自完成的方式推动该领域向前发展。”下面讲述一组研究人员与 NSABP 合作验证其基因表达特征的故事非常鼓舞人心。但这不是科学运作的唯一方式。研究人员不应受限于一种或另一种模型。有时合作是必要的，有时则不是，但在任何情况下我们都不应该给研究人员贴上 “共生” 或“寄生”的标签，这些术语具有极端的含义。
4.  **“How would data sharing work best? We think it should happen symbiotically, not parasitically.”** I think that it should happen _automatically_. If you generate a data set with public funds, you should be required to immediately make it available to researchers in the community. But you should _get credit for generating the data set and the hypothesis that led to the data set_. The problem is that people who generate data will almost never be as fast at analyzing it as people who know how to analyze data. But both deserve credit, whether they are working together or not.  
    “数据共享如何发挥最佳效果？我们认为这应该是共生的，而不是寄生的。” 我认为它应该自动发生。如果您使用公共资金生成数据集，您应该被要求立即将其提供给社区的研究人员。但是你应该因生成数据集和导致数据集的假设而受到赞扬。问题在于生成数据的人在分析数据时几乎永远不会像知道如何分析数据的人那样快。但无论他们是否合作，都值得赞扬。
5.  **“Start with a novel idea, one that is not an obvious extension of the reported work. Second, identify potential collaborators whose collected data may be useful in assessing the hypothesis and propose a collaboration. Third, work together to test the new hypothesis. Fourth, report the new findings with relevant coauthorship to acknowledge both the group that proposed the new idea and the investigative group that accrued the data that allowed it to be tested.”** The trouble with this framework is that it preferentially accrues credit to data generators and doesn’t accurately describe the role of either party. To flip this argument around, you could just as easily say that anyone who uses [Steven Salzberg](http://salzberg-lab.org/)’s software for aligning or assembling short reads should make him a co-author. I think Dr. Drazen would agree that not everyone who aligned reads should add Steven as co-author, despite his contribution being critical for the completion of their work.  
    “从一个新颖的想法开始，一个不是报告工作的明显延伸的想法。其次，确定潜在的合作者，其收集的数据可能有助于评估假设并提出合作建议。第三，共同检验新假设。第四，与相关合著者一起报告新发现，以感谢提出新想法的小组和收集数据以对其进行测试的调查小组。” 这个框架的问题在于它优先将信用归于数据生成者并且没有准确描述任何一方的角色。为了扭转这个争论，你可以很容易地说，任何使用 Steven Salzberg 的软件来比对或组装短读的人都应该让他成为合著者。我认为 Dr. Drazen 会同意，并非所有对齐阅读的人都应该将 Steven 添加为合著者，尽管他的贡献对于完成他们的工作至关重要。

#### How you should format your data  
你应该如何格式化你的数据

For maximum speed in the analysis this is the information you should share:  
为了在分析中获得最大速度，这是您应该分享的信息：

1.  The raw data. 原始数据。
2.  A [tidy data set](http://vita.had.co.nz/papers/tidy-data.pdf)  
    整齐的数据集
3.  A code book describing each variable and its values in the tidy data set.  
    描述整洁数据集中每个变量及其值的代码手册。
4.  An explicit and exact recipe you used to go from 1 -> 2,3  
    您曾经从 1 -> 2,3 获得的明确而准确的食谱
...
*   The complicated [JSON](http://en.wikipedia.org/wiki/JSON) data you got from scraping the [Twitter API](https://twitter.com/twitterapi)  
    从 Twitter API 中获取的复杂 JSON 数据
*   The hand-entered numbers you collected looking through a microscope  
    您通过显微镜观察收集到的手写数字

You know the raw data is in the right format if you:  
如果您满足以下条件，您就知道原始数据的格式正确：
...
如果您对数据进行了任何操作，那么它就不是数据的原始形式。将被操纵的数据报告为原始数据是减慢分析过程的一种非常常见的方法，因为分析师通常必须对您的数据进行取证研究才能弄清楚为什么原始数据看起来很奇怪。

#### The tidy data set 整洁的数据集
...
整理数据的一般原则由 Hadley Wickham 在本文和视频中阐述。论文和视频都着重介绍了 R 包，您可能知道也可能不知道如何使用。无论如何，您应该注意的四项一般原则是：

1.  Each variable you measure should be in one column  
    您测量的每个变量都应该在一列中
2.  Each different observation of that variable should be in a different row  
    该变量的每个不同观察值都应该在不同的行中
3.  There should be one table for each “kind” of variable  
    每种 “种类” 的变量应该有一个表
4.  If you have multiple tables, they should include a column in the table that allows them to be linked  
    如果你有多个表，它们应该在表中包含一个允许它们链接的列
...
虽然这些是硬性规定，但还有许多其他因素可以使您的数据集更易于处理。首先是在每个数据表 / 电子表格的顶部包含一行，其中包含完整的行名称。因此，如果您测量患者诊断时的年龄，您会在该列的开头使用名称 `AgeAtDiagnosis` 而不是 `ADx` 或其他人可能难以理解的其他缩写。
...
这是一个例子，说明这将如何从基因组学中发挥作用。假设您使用 RNA 测序收集了 20 个人的基因表达测量值。您还收集了有关患者的人口统计和临床信息，包括他们的年龄、治疗和诊断。您将拥有一张包含临床 / 人口统计信息的表格 / 电子表格。它将有四列（患者 ID、年龄、治疗、诊断）和 21 行（一行带有变量名称，然后每一行代表每个患者）。您还将拥有一份汇总基因组数据的电子表格。通常这种类型的数据在每个外显子的计数水平上进行总结。假设您有 100,000 个外显子，那么您将拥有一个包含 21 行（一行用于基因名称，一行用于每个患者）和 100,001 列（一行用于患者 ID，一行用于每种数据类型）的表格 / 电子表格。
...
如果您在 Excel 中与协作者共享您的数据，则整洁的数据应该在每个表的一个 Excel 文件中。他们不应该有多个工作表，不应该对数据应用宏，也不应该突出显示任何列 / 单元格。或者以 CSV 或制表符分隔的文本文件共享数据。
...
对于几乎所有的数据集，您计算的测量值都需要比您潜入电子表格更详细的描述。密码本包含此信息。它至少应包含：

1.  Information about the variables (including units!) in the data set not contained in the tidy data  
    有关数据集中变量（包括单位！）的信息不包含在整齐的数据中
...
在我们的基因组学示例中，分析人员想知道每个临床 / 人口统计变量的测量单位是什么（年龄，按名称 / 剂量治疗，诊断水平和异质性）。他们还想知道您是如何挑选用于总结基因组数据（UCSC/Ensembl 等）的外显子的。他们还想知道有关您如何进行数据收集 / 研究设计的任何其他信息。例如，这些是前 20 名走进诊所的患者吗？他们是根据某些特征（例如年龄）精心挑选的 20 名患者吗？他们是随机接受治疗的吗？
...
此文档的通用格式是 Word 文件。应该有一个名为 “研究设计” 的部分，其中对您如何收集数据进行了详尽的描述。有一个称为 “代码簿” 的部分描述了每个变量及其单位。

#### How to code variables 如何编码变量
...
当您将变量放入电子表格时，根据它们的数据类型，您将遇到几个主要类别：
...
连续变量是在定量尺度上测量的任何东西，可以是任何小数。举个例子，比如以公斤为单位的重量。序数数据是具有固定的、少量（< 100）但有序的数据。例如，这可能是调查答复，其中的选择是：差、一般、好。分类数据是有多个类别但未排序的数据。一个例子是性别：男性或女性。缺失数据是缺失的数据，你不知道机制。您应该将缺失值编码为 `NA` 。截尾数据是您在某种程度上知道缺失机制的数据。常见的例子是测量值低于检测限或患者失访。当您没有数据时，它们也应该编码为 `NA` 。但是，您还应该在整洁的数据中添加一个名为 “VariableNameCensored” 的新列，如果经过审查，该列的值应为 `TRUE` ，否则应为 `FALSE` 。在代码手册中，您应该解释为什么缺少这些值。如果您知道某些数据丢失是有原因的，那么向分析师报告是绝对重要的。你也不应该归咎于 / 弥补 / 丢弃缺失的观察结果。
...
通常，尽量避免将分类或有序变量编码为数字。当您在 tidy 数据中输入性别值时，它应该是 “男性” 或“女性”。数据集中的序数值应该是 poor、“fair”和“good”，而不是 1、2、3。这将避免关于影响方向的潜在混淆，并有助于识别编码错误。

Always encode every piece of information about your observations using text. For example, if you are storing data in Excel and use a form of colored text or cell background formatting to indicate information about an observation (“red variable entries were observed in experiment 1.”) then this information will not be exported (and will be lost!) when the data is exported as raw text. Every piece of data should be encoded as actual text that can be exported.  
始终使用文本对有关您的观察结果的每条信息进行编码。例如，如果您将数据存储在 Excel 中并使用彩色文本或单元格背景格式来指示有关观察的信息（“实验 1 中观察到红色变量条目。”），则此信息将不会导出（并且将丢失！）当数据导出为原始文本时。每条数据都应编码为可以导出的实际文本。

#### The instruction list/script 指令列表 / 脚本
...
您可能以前听说过，但是可重复性在计算科学中是一个大问题。这意味着，当您提交论文时，审稿人和世界其他地方应该能够准确地复制从原始数据到最终结果的分析。如果你想提高效率，你可能会在数据被认为是整洁的之前执行一些总结 / 数据分析步骤。
...
执行汇总时最理想的做法是创建一个计算机脚本（在 `R` 、 `Python` 或其他格式中），将原始数据作为输入并生成您共享的整洁数据作为输出。您可以尝试运行您的脚本几次，看看代码是否产生相同的输出。
...
在许多情况下，收集数据的人有动机让统计人员整理数据以加快协作过程。他们可能不知道如何使用脚本语言编写代码。在那种情况下，您应该向统计学家提供的是一种叫做伪代码的东西。它应该看起来像：

1.  Step 1 - take the raw file, run version 3.1.2 of summarize software with parameters a=1, b=2, c=3  
    第 1 步 - 获取原始文件，使用参数 a=1、b=2、c=3 运行 3.1.2 版汇总软件
...
3.  Step 3 - take column three of outputfile.out for each sample and that is the corresponding row in the output data set  
    第 3 步 - 为每个样本取 outputfile.out 的第三列，即输出数据集中的对应行
...
您还应该包括有关您在哪个系统（Mac/Windows/Linux）上使用该软件以及您是否多次尝试以确认它给出相同结果的信息。理想情况下，您将由同学 / 实验室伙伴运行此程序，以确认他们可以获得与您所做的相同的输出文件。

_Note:_ Parts of this chapter appeared in the Simply Statistics blog posts [“PLoS One, I have an idea for what to do with all your profits: buy hard drives”](http://simplystatistics.org/2014/03/05/plos-one-i-have-an-idea-for-what-to-do-with-all-your-profits-buy-hard-drives/) and [“On research parasites and internet mobs - let’s try to solve the real problem.”](http://simplystatistics.org/2016/01/25/on-research-parasites-and-internet-mobs-lets-try-to-solve-the-real-problem/) as well as the [Leek group guide to data sharing](https://github.com/jtleek/datasharing)  
注意：本章的部分内容出现在 Simply Statistics 博客文章 “PLoS One，我知道如何处理你所有的利润：购买硬盘” 和“关于研究寄生虫和互联网暴民 - 让我们尝试解决真正的问题”以及 Leek 组数据共享指南

Scientific blogging 科学博客
------------------------
...
你博客的多少很大程度上取决于博客的目的。如果您只是将它用作回应批评的地方，那么您根本不需要经常写博客。如果您使用它来提升您的知名度，那么更频繁地发帖会很有用，这样人们就会开始关注您的博客。但要小心，因为存在能量权衡——如果你写博客，你将没有那么多精力来写论文和做研究。
...
我认为处理能量权衡的最佳方法是与其他人一起创建博客。如果您有合著者，那么您可以保持更连续的内容流，而不必独自负责生成所有这些帖子。我认为拥有一个 2-5 人的博客合作社可以在分配能量和确保您获得整个博客的最大荣誉之间取得很好的平衡。但是，即使是更大的合作社也可以真正有用地吸引读者，同时最大限度地减少您的工作量。

**Blog to respond to criticism  
回应批评的博客**

A recent paper by [Case and Deaton](http://www.pnas.org/content/early/2015/10/29/1518393112.full.pdf) on death rates for middle class people. was discussed by [Andrew Gelman](http://www.slate.com/articles/health_and_science/science/2015/11/death_rates_for_white_middle_aged_americans_are_not_increasing.html) among many others. They noticed a potential bias in the analysis and did some re-analysis. There has been [Roger and [a lot of discussion around this paper](http://www.pnas.org/content/early/2015/10/29/1518393112.full.pdf) on death rates for middle class people. Whenever this happens someone points out how academics are often [surprised by the speed and ferocity of the criticism of their work](http://noahpinionblog.blogspot.com/2015/11/gelman-vs-case-deaton-academics-vs.html). The best way to respond is to have a blog where you can relatively quickly publish long-form responses to any criticism that is raised about your work.  
Case 和 Deaton 最近发表的一篇关于中产阶级死亡率的论文。 Andrew Gelman 和其他许多人讨论过。他们注意到分析中存在潜在偏差，并进行了一些重新分析。围绕这篇关于中产阶级死亡率的论文，已经有 [罗杰和很多讨论。每当发生这种情况时，就会有人指出，学者们常常对他们工作的批评速度和猛烈程度感到惊讶。最好的回应方式是拥有一个博客，您可以在其中相对快速地发布对对您的工作提出的任何批评的长篇回应。
...
您还可以使用您的博客来提高您在学术和非学术社区中的知名度。在学术界，批评论文或提出新想法的技术帖子可能是最有价值的。在学术界之外，博客可能更为重要。它可以成为人们发现您的有用方式。
...
根据我个人的经验，许多公司已经根据我的博文找到了我。我的几个学生也使用他们的博客来帮助提高他们的知名度并在行业中建立联系。就求职而言，对提升形象最有用的帖子类型往往是：
...
你为什么写博客（图片来自 https://xkcd.com/386/）
...
博客是记录您正在学习或从事的工作的好地方。它也可以是从事非主要论文项目的好地方。一般来说，研究工作需要长时间专注于一个非常具体的问题。这可能会令人沮丧，因为它限制了您的思考范围，而且这些类型的项目需要很长时间才能完成。有时拥有一个博客是件好事，这样您就可以从事短期和不同的项目，以获得成就感或它提供的多样性。
...
这又取决于您希望使用您的博客来提高您的个人资料的程度，而不是仅仅定期使用。如果你只是偶尔写一篇文章，我会推荐一个非常简单的博客平台，比如

*   [Medium](https://medium.com/) - widely popular with general audiences and very easy to set up  
    中等 - 广受普通观众欢迎并且非常容易设置
*   [Blogger](http://www.blogger.com/) - relatively easy to manage, integrates nicely with Google accounts  
    Blogger - 相对易于管理，与 Google 帐户完美集成
*   [Wordpress.org](https://wordpress.org/) - slightly less intuitive than Medium or Blogger, but with increased flexibility through plugins  
    Wordpress.org - 比 Medium 或 Blogger 稍微不直观，但通过插件增加了灵活性
...
如果您打算建立一个合作的博主群或定期写博客以提高您的知名度，您可以考虑其他平台，例如

*   A self-hosted [Wordpress](https://wordpress.org/) blog. This requires you to set up hosting and handle more yourself, but gives you some freedom of control and opens up some additional plugins.  
    一个自托管的 Wordpress 博客。这需要您自己设置托管和处理更多内容，但可以让您自由控制并打开一些额外的插件。
*   A blog hosted on Github using something like [Jekyll](http://jekyllbootstrap.com/) which can be used to easily integrate the contributions of many folks. This will require some technical expertise.  
    在 Github 上托管的一个博客，使用类似 Jekyll 的东西，可以用来轻松地整合许多人的贡献。这将需要一些技术专长。

In general I would start with the simpler tools and then only move to more complicated tools as they become necessary.  
一般来说，我会从更简单的工具开始，然后只在必要时才转向更复杂的工具。
...
Many times that folks have been confronted with blogs or the quick publication process of [f1000Research](http://f1000research.com/). I think it is pretty scary for folks who aren’t used to “internet speed” to see this play out and I thought it would be helpful to make a few points.  
人们多次遇到博客或 f1000Research 的快速发布过程。我认为对于那些不习惯 “互联网速度” 的人来说，看到这个结果是非常可怕的，我认为提出几点会有所帮助。
...
1.  **Everyone is an internet scientist now.** The internet has arrived as part of academics and if you publish a paper that is of interest (or if you are a Nobel prize winner, or if you dispute a claim, etc.) you will see discussion of that paper within a day or two on the blogs. This is now a fact of life.  
    现在每个人都是互联网科学家。互联网作为学术的一部分已经到来，如果你发表了一篇感兴趣的论文（或者如果你是诺贝尔奖获得者，或者如果你对某项主张提出异议等），你将在一两天内看到对该论文的讨论在博客上。现在这是生活中的事实。
2.  **The internet loves a fight** The internet responds best to personal/angry blog posts or blog posts about controversial topics like p-values, errors, and bias. Almost certainly if someone writes a blog post about your work or an f1000 paper it will be about an error/bias/correction or something personal.  
    互联网喜欢打架互联网对个人 / 愤怒的博客文章或关于有争议的话题（如 p 值、错误和偏见）的博客文章反应最好。几乎可以肯定，如果有人写了一篇关于你的工作的博客文章或一篇 f1000 论文，那将是关于错误 / 偏见 / 更正或个人的事情。
3.  **Takedowns are easier than new research and happen faster**. It is much, much easier to critique a paper than to design an experiment, collect data, figure out what question to ask, ask it quantitatively, analyze the data, and write it up. This doesn’t mean the critique won’t be good/right it just means it will happen much much faster than it took you to publish the paper because it is easier to do. All it takes is noticing one little bug in the code or one error in the regression model. So be prepared for speed in the response.  
    取缔比新研究更容易，而且发生得更快。批评一篇论文比设计实验、收集数据、找出要问的问题、定量地提出问题、分析数据并写下来要容易得多。这并不意味着批评不会很好 / 不正确，它只是意味着它会比你发表论文要快得多，因为它更容易做到。只需注意代码中的一个小错误或回归模型中的一个错误即可。因此，请做好响应速度的准备。
...
鉴于这三件事，如果你写了一篇有趣的论文并且人们正在讨论它 - 他们肯定会这样做（第 1 点），你有几个选择如何反应，这可能会让你感到不舒服（第 1 点） 2)，并且比您预期的要快（第 3 点）。首先要记住的是，互联网要你 “反击”，要宣布 “赢家”。阅读关于友好分歧的文章并不能吸引观众。这就是为什么有真人秀节目。所以你会有得分的压力，要聪明，要快，反驳每一分或被宣布为失败者。我从自己的经历中发现，这也是我想做的事情。我认为抵制这种冲动既是 (a) 非常非常困难，也是 (b) 正确的做法。我发现最好的解决办法是为你的工作感到自豪，但要谦虚，因为没有论文是完美的，这没关系。如果你尽力而为，明智的人会承认这一点。

I think these are the three ways to respond to rapid internet criticism of your work.  
我认为这是回应网络上对你作品的快速批评的三种方式。
...
*   **Option 1: Respond on internet time.** This means if you publish a big paper that you think might be controversial you should block off a day or two to spend time on the internet responding. You should be ready to do new analysis quickly, be prepared to admit mistakes quickly if they exist, and you should be prepared to make it clear when there aren’t. You will need social media accounts and you should probably have a blog so you can post longer form responses. Github/Figshare accounts make it better for quickly sharing quantitative/new analyses. Again your goal is to avoid the personal and stick to facts, so I find that Twitter/Facebook are best for disseminating your more long form responses on blogs/Github/Figshare. If you are going to go this route you should try to respond to as many of the major criticisms as possible, but usually they cluster into one or two specific comments, which you can address all in one.  
    选项 1：响应互联网时间。这意味着如果你发表了一篇你认为可能有争议的大论文，你应该花一两天时间在互联网上回应。你应该准备好快速进行新的分析，准备好在错误存在时迅速承认错误，并且在没有错误时准备好澄清错误。您将需要社交媒体帐户，并且您可能应该拥有一个博客，以便您可以发布更长的回复。 Github/Figshare 帐户可以更好地快速共享定量 / 新分析。同样，您的目标是避免个人问题并坚持事实，因此我发现 Twitter/Facebook 最适合在博客 / Github/Figshare 上传播您的较长形式的回复。如果你打算走这条路，你应该尝试回应尽可能多的主要批评，但通常它们会聚集成一两个特定的评论，你可以将它们集中在一起。
*   **Option2 : Respond in academic time.** You might have spent a year writing a paper to have people respond to it essentially instantaneously. Sometimes they will have good points, but they will rarely have carefully thought out arguments given the internet-speed response (although remember point 3 that good critiques can be faster than good papers). One approach is to collect all the feedback, ignore the pressure for an immediate response, and write a careful, scientific response which you can publish in a journal or in a fast outlet like f1000Research. I think this route can be the most scientific and productive if executed well. But this will be hard because people will treat that like “you didn’t have a good answer so you didn’t respond immediately”. The internet wants a quick winner/loser and that is terrible for science. Even if you choose this route though, you should make sure you have a way of publicizing your well thought out response - through blogs, social media, etc. once it is done.  
    选项 2：在学术时间内做出回应。你可能花了一年时间写一篇论文，让人们基本上立即做出回应。有时他们会有很好的观点，但鉴于互联网速度的响应，他们很少会仔细考虑论点（尽管请记住第 3 点，好的评论可能比好的论文更快）。一种方法是收集所有反馈，忽略立即响应的压力，并写出仔细、科学的响应，您可以将其发表在期刊或 f1000Research 等快速媒体上。如果执行得好，我认为这条路线是最科学和最有成效的。但这将很难，因为人们会认为 “你没有很好的答案，所以你没有立即回应”。互联网想要一个快速的赢家 / 输家，这对科学来说是可怕的。不过，即使您选择了这条路线，您也应该确保有一种方式来宣传您深思熟虑的回应——一旦完成，就可以通过博客、社交媒体等方式进行宣传。
*   __ Option 3: Do not respond.__ This is what a lot of people do and I’m unsure if it is ok or not. Clearly internet facing commentary can have an impact on you/your work/how it is perceived for better or worse. So if you ignore it, you are ignoring those consequences. This may be ok, but depending on the severity of the criticism may be hard to deal with and it may mean that you have a lot of questions to answer later. Honestly, I think as time goes on if you write a big paper under a lot of scrutiny Option 3 is going to go away.  
    __ 选项 3：不回应。__ 这是很多人所做的，我不确定是否可以。显然，面向互联网的评论会影响您 / 您的工作 / 人们对它的看法是好是坏。所以如果你忽略它，你就忽略了这些后果。这可能没问题，但根据批评的严重程度，可能很难处理，这可能意味着您以后有很多问题要回答。老实说，我认为随着时间的推移，如果你在大量审查下写一篇大论文，选项 3 将会消失。
...
只有当你写了一篇很多人关心 / 有争议的论文时，所有这一切才适用。许多技术论文不会有这个问题，如果你的声明很小，这也可能不适用。也就是说，执行选项 3 越来越难。回应批评是解决重大科学问题的重要组成部分。使用博客可以是一种有效、快速且可控的方式来响应您的论文中提出的问题。

#### Controversy 争议

Be aware that blogs are public and can end up widely distributed. A few years ago I wrote a [blog post](http://simplystatistics.org/2012/11/26/the-statisticians-at-fox-news-use-classic-and-novel-graphical-techniques-to-lead-with-data/) that pointed out some ways that a particular media organization had distorted data. I wasn’t trying to be political, just poke fun at the statistical curiosity in the media. But the post quickly went viral and ended up leading to a large amount of interesting email/responses. In general, be aware that if you blog about a controversial topic you may end up having lots of people read it. So if you are going to talk about controversial topics you should do it deliberately. It doesn’t mean you shouldn’t blog about important/controversial issues but you should be aware of the attention you may get.  
请注意，博客是公开的，最终可能会广泛传播。几年前，我写了一篇博客文章，指出了特定媒体组织扭曲数据的一些方式。我不是想搞政治，只是取笑媒体对统计数据的好奇心。但该帖子迅速传播开来，并最终引发了大量有趣的电子邮件 / 回复。一般来说，请注意，如果您的博客涉及一个有争议的话题，您最终可能会有很多人阅读它。所以如果你要谈论有争议的话题，你应该故意这样做。这并不意味着你不应该在博客上讨论重要 / 有争议的问题，但你应该意识到你可能会受到关注。

#### Unprofessional content 不专业的内容
...
一般来说，您应该将您的科学博客保留用于专业写作。请记住，您的博客将反映您作为专业人士和科学家的身份。如果您发表粗鲁、研究不足或愤怒的帖子——这可能是人们对您的假设。当您申请工作、申请补助金或申请晋升时，您希望您的所有写作都能很好地反映您。
...
拥有博客（也是最容易写的帖子）的最常见原因之一是删除其他人的工作和博客帖子。请记住，仅仅因为这是一条简单的路线并且有时可能会导致网页浏览，所以并不总是最好的做法。请记住，简单的批评可能会给接受方的人带来大麻烦。这并不意味着不要批评，但在这样做时要注意你的语气和意图，以免被塑造成 “只是另一个愤怒的博主”。

_Note:_ Parts of this chapter appeared in the Simply Statistics blog posts [“So you are getting crushed on the internet? The new normal for academics.”](http://simplystatistics.org/2015/11/16/so-you-are-getting-crushed-on-the-internet-the-new-normal-for-academics/)  
注意：本章的部分内容出现在 Simply Statistics 博客文章 “So you are getting crushed on the internet? 学术界的新常态。”

Scientific code 科学守则
--------------------
...
代码和数据一样，应该自由分发。代码可以包括 (a) 用于执行特定于论文的分析的软件或 (b) 用于执行更一般分析的软件。

**Post your code to a public repository  
将您的代码发布到公共存储库**

You should post your data to a public repository. You will eventually change websites and managing your own data can be a hassle. Take advantage of field specific repositories or more general code sharing services like to make your code available.  
您应该将数据发布到公共存储库。您最终会更换网站，管理自己的数据可能会很麻烦。利用特定领域的存储库或 Github 等更通用的代码共享服务来提供您的代码。
...
现在在网上发布你的代码相对简单，但通常代码是完全随意存放的。原因是历史上很少鼓励以深思熟虑或最有用的方式共享代码。为了最大化您的数据的价值和您的影响 (a) 将您的代码发布到一个开放的存储库，(b) 记录您用于特定分析的代码版本，(c) 使用 README 文件、评论记录和解释您的代码，和文学编程。
...
令人惊讶的是，与特定论文相关的代码经常不容易被发现，即使它们是公开的。确保在您的出版物中包含指向您的代码的直接和永久链接，以及相关的版本号。

**When sharing software, use repositories with review  
共享软件时，使用经过审查的存储库**

It is very easy to share Python modules or R packages online through Github. But there is no process for vetting these software on general code repositories. It will dramatically improve the credibility and use of your software if you post them to repositories with some level of code review such as [CRAN](https://cran.r-project.org/) or [Bioconductor](http://bioconductor.org/).  
通过 Github 在线共享 Python 模块或 R 包非常容易。但是没有在通用代码存储库上审查这些软件的过程。如果您将它们发布到具有一定级别代码审查的存储库（例如 CRAN 或 Bioconductor），它将显着提高软件的可信度和使用。

### Scientific code - what tools should I use?  
科学代码——我应该使用什么工具？

#### Posting general code 发布通用代码

For posting general code and software use any system that is public, shareable, and version controlled. The two most common options are [Github](http://github.com/) and [Bitbucket](https://bitbucket.org/). Do not share your code via scripts stored in a non-version controlled directory like Dropbox or in a non-plain text format like pdf.  
要发布通用代码和软件，请使用任何公共、可共享和版本控制的系统。两个最常见的选项是 Github 和 Bitbucket。不要通过存储在非版本控制目录（如 Dropbox）或非纯文本格式（如 pdf）中的脚本共享你的代码。

#### Posting software 发帖软件

For posting software that will be used by the community you can start with [Github](http://github.com/) and [Bitbucket](https://bitbucket.org/) for development and getting off the ground quickly. At some point, to make your software broadly available to the community you should consider distributing it through a curated repository.  
对于将由社区使用的发布软件，您可以从 Github 和 Bitbucket 开始开发并快速启动。在某些时候，为了使您的软件在社区中广泛可用，您应该考虑通过精选存储库分发它。

*   **R** - several curated repositories exist including [CRAN](https://cran.r-project.org/) and [rOpenSci](http://ropensci.org/) for general purpose software and [Bioconductor](http://bioconductor.org/) for genomics software.  
    R - 存在多个精选存储库，包括用于通用软件的 CRAN 和 rOpenSci 以及用于基因组学软件的 Bioconductor。
*   **Python** - there is an index of libraries at [PyPI](https://pypi.python.org/pypi)  
    Python - PyPI 上有一个库索引
*   __Galaxy_ - you can create Galaxy workflows and share them through the [Galaxy Toolshed](https://wiki.galaxyproject.org/GetInvolved?action=show&redirect=Get+Involved#Develop_Tools_and_Tool_Definitions)  
    _Galaxy - 您可以创建 Galaxy 工作流并通过 Galaxy Toolshed 共享它们

#### Posting analysis code 贴出分析代码

There are two main approaches to sharing code: through a collection of scripts and through literate programming documents.  
共享代码有两种主要方法：通过脚本集合和通过文学编程文档。

**Code and scripts 代码和脚本**

The most basic way to share code is through plain text code files. This is true regardless of what language you use. When writing code and distributing as scripts you should use version control software to keep track of the code you are using in the version of the analysis that you publish. Any text editor will do, but the best software integrate directly with Github/Bitbucket and include code highlighting. [Emacs](https://www.gnu.org/software/emacs/) is a good general purpose code editor. You can also use online editors such as [Cloud9](https://c9.io/). If you are coding in R, [Rstudio](https://www.rstudio.com/) is a good option.  
共享代码的最基本方式是通过纯文本代码文件。无论您使用何种语言，都是如此。在编写代码并作为脚本分发时，您应该使用版本控制软件来跟踪您在发布的分析版本中使用的代码。任何文本编辑器都可以，但最好的软件直接与 Github/Bitbucket 集成并包括代码突出显示。 Emacs 是一个很好的通用代码编辑器。您还可以使用 Cloud9 等在线编辑器。如果您使用 R 编写代码，Rstudio 是一个不错的选择。

**Literate programming 文学编程**

Literate programming documents are documents that integrate code and text into a single document so that you can easily explain the results in the same document. The two most widely used literate programming tools are [knitr](http://yihui.name/knitr/) and [rmarkdown](http://rmarkdown.rstudio.com/) for R and [Jupyter notebooks](http://jupyter.org/) that can be used with a wide range of programming languages. An advantage of literate programming is that you can make the documents more fully explain the analysis you have done, while leaving your work completely reproducible.  
文字编程文档是将代码和文本集成到单个文档中的文档，以便您可以轻松地在同一文档中解释结果。两种最广泛使用的文学编程工具是 knitr 和 rmarkdown，用于 R 和 Jupyter 笔记本，可用于多种编程语言。文学编程的一个优点是您可以使文档更全面地解释您所做的分析，同时让您的工作完全可重现。
...
**Commenting and documenting 评论和记录**
...
在平面脚本文件中，最佳方式是使用注释来描述代码的作用。在 R markdown 文档或 Jupyter 笔记本等文字编程文档中，一种改进的替代方法是使用简单的英语来描述你在做什么。留下好的和有用的评论的关键是解释：（a）你为什么写这段代码，（b）代码是如何工作的，以及（c）它应该产生什么。

**Reporting versions 报告版本**
...
当您编写用于分析的文档时，您应该跟踪您使用的软件版本并在您的任何出版物中报告它。如果您使用的是 Github 或 Bitbucket，您可以在您的文件中找到生成结果的软件版本的提交编号。
...
分析代码应包括一条命令，用于打印分析期间使用的硬件、软件平台和软件包或库的所有版本。这样，即使软件版本发生变化，人们也可以返回并重建您的分析。

#### Software packages 软件包

**Naming your software 命名您的软件**

The first step in creating your software is to give it a name. Hadley Wickham [has some ideas](http://adv-r.had.co.nz/Package-basics.html) about it. Here are some useful rules:  
创建软件的第一步是给它起一个名字。 Hadley Wickham 对此有一些想法。以下是一些有用的规则：
...
*   Make sure there is no library or package with the same name on the repository you will submit to  
    确保您要提交到的存储库中没有同名的库或包
*   No underscores, dashes or any other special characters/numbers  
    没有下划线、破折号或任何其他特殊字符 / 数字
*   Make it all lower case - people hate having to figure out caps in names of packages.  
    全部小写——人们讨厌必须弄清楚包裹名称中的大写字母。
*   Make it memorable; if you want serious people to use it don’t be too cute.  
    让人难忘；如果您想让严肃的人使用它，请不要太可爱。
*   Make it as short as you possibly can while staying googleable.  
    在保持 googleable 的同时尽可能地缩短它。

**Versioning your software 对软件进行版本控制**

There are a number of potential code conventions that can be used when versioning your software. One useful idea is to think about development and release versions of your software. Release is the stable version that everyone is using and development is where you make all of your potentially experimental changes. [Bioconductor](http://www.bioconductor.org/) has a useful [versioning scheme](http://www.bioconductor.org/developers/version-numbering/) which can be augmented with ith some helpful suggestions from [Kasper Hansen](http://www.biostat.jhsph.edu/~khansen/).  
在对软件进行版本控制时，可以使用许多潜在的代码约定。一个有用的想法是考虑软件的开发和发布版本。发布是每个人都在使用的稳定版本，而开发是您进行所有潜在实验性更改的地方。 Bioconductor 有一个有用的版本控制方案，可以通过 Kasper Hansen 的一些有用建议对其进行扩充。

To use this system, the format of the version number will always be `x.y.z`. When you start any new package the version number should be `0.1.0`. Every time you make any change public (e.g., push to [GitHub](https://github.com/)) you should increase `z` in the version number. If you are making local commits but not making them public to other people you don’t need to increase `z`. You should stay in version `0.1.z` basically up until you are ready to submit to a repository.  
要使用这个系统，版本号的格式总是 `x.y.z` 。当您启动任何新包时，版本号应为 `0.1.0` 。每次公开任何更改（例如，推送到 GitHub ）时，您应该增加版本号中的 `z` 。如果您正在本地提交但不将它们公开给其他人，则不需要增加 `z` 。在您准备好提交到存储库之前，您应该基本上停留在版本 `0.1.z` 。
...
在发布之前，如果您对功能的组织或使用方式进行了重大重新设计，则可以增加 `y` 。你不应该在发布前增加 `x` 。
...
第一次发布时，您会将版本号更改为 `1.0.0` 。发布后，如果您打算继续在该项目上工作，您应该将您的开发版本升级到 `1.1.0` 。
...
但是如果你的软件解决了一个真正的问题（它应该！）并且有很好的文档记录（它会！）那么人们就会使用它并且你会对世界产生积极的影响。

Documentation has two main components. The first component is help files. The second component is vignettes or tutorials.  
文档有两个主要组成部分。第一个组件是帮助文件。第二个组成部分是小插图或教程。

**Help files 帮助文件**

These files document each of the functions/methods/classes you will expose to your users. You should document (a) all inputs to the functions, (b) all outputs, and (c) expected behavior of the function under common circumstances. You  
这些文件记录了您将向用户公开的每个函数 / 方法 / 类。您应该记录 (a) 函数的所有输入，(b) 所有输出，以及 (c) 函数在常见情况下的预期行为。你  
should also give examples of how to use them.  
还应该举例说明如何使用它们。

**Vignettes 小插曲**

Documentation in the help files is important and is the primary way that people will figure out your functions if they get stuck. But it is equally (maybe more) critical that you help people _get started_. The way that you do that is to create a vignette. A vignette is a tutorial that includes the following components:  
帮助文件中的文档很重要，是人们在遇到困难时了解您的功能的主要方式。但帮助人们开始工作同样（也许更重要）至关重要。你这样做的方法是创建一个小插图。小插图是包含以下组件的教程：
...
*   One or more example analyses with  
    一个或多个示例分析
    *   A small, real data set 一个小的真实数据集
...
小插图是使您的软件得到广泛使用的重要组成部分。他们会让人们开始使用你的包裹，并为他们提供一个起点。任何你希望被广泛使用的软件都应该包括一个 Vignette 或教程。

#### [Good writers borrow from other authors, great authors steal outright  
好作家从其他作家那里借鉴，伟大的作家直接窃取](http://www.brainyquote.com/quotes/quotes/a/aaronsorki405048.html)

One thing I think a lot of academics (definitely including myself here) struggle with is the need to be “new” and “innovative” with everything they do. There is a strong [selective pressure](http://en.wikipedia.org/wiki/Evolutionary_pressure) for these qualities in academia.  
我认为很多学者（当然包括我自己）都在努力解决的一件事是，他们所做的每一件事都需要 “新颖” 和“创新”。学术界对这些品质有很强的选择压力。

But when writing software it is very, very important not to reinvent every single wheel you see. One person who is awesome at blending existing tools and exponentially building value is [Ramnath Vaidyanathan](https://github.com/ramnathv). He built [slidify](https://github.com/ramnathv/slidify) on top of [knitr](http://cran.r-project.org/web/packages/knitr/index.html) and [rCharts](https://github.com/ramnathv/rCharts) on top of existing [D3](http://d3js.org/) libraries. They allowed him to create awesome software without having to solve every single problem.  
但是在编写软件时，不要重新发明你看到的每一个轮子是非常非常重要的。 Ramnath Vaidyanathan 是一位擅长混合现有工具并以指数方式创造价值的人。他在 knitr 之上构建了 slidify，在现有 D3 库之上构建了 rCharts。他们让他能够创造出很棒的软件，而不必解决每一个问题。
...
对于每个创建代码的人来说，一个主要的诱惑是生成一堆他们认为用户会想要的功能，而无需实际测试这些功能。问题是您在包中创建的每个新功能都会单调地增加依赖项的数量和您必须维护的代码量。一般来说，原则应该是创建足够的功能，用户可以安装你的包，执行你的分析，并返回结果，没有多余的功能。

Specifically, be wary of things like GUIs or [Shiny](http://www.rstudio.com/shiny/) apps. Given the heavy emphasis placed on reproducibility these days, it is rarely the case that real/important analyses will be performed in a point and click format.  
具体来说，要警惕 GUI 或 Shiny 应用程序之类的东西。鉴于如今对可重复性的高度重视，很少有实际 / 重要的分析会以点击格式进行。
...
如果您正在创建点击式用户会感兴趣的产品，我很乐意在这方面支持您，因为我认为这些东西很酷，可能是未来，并且肯定会提高您对工作的兴趣. 但是它们在维护方面存在潜在的主要困难，应该放在单独的包中，由您自己承担。

_Note_: Part of this chapter appeared in the [Leek group guide to developing R packages](https://github.com/jtleek/rpackages)  
注：本章部分内容出现在 Leek group guide to developing R packages

Social media in science 科学中的社交媒体
--------------------------------
...
社交媒体可以为现代科学家发挥多种作用。在这里，我将重点关注社交媒体对主要关注点不是科学传播的在职科学家的作用。刚开始使用社交媒体的人经常会忽略的一点是，要发展成功的社交媒体存在有两个独立的组成部分。
...
首先是发展您社区中的追随者和联系。这是通过成为内容策展人、内容生成者或以其他方式变得有趣 / 有趣来实现的。这通常与您的科学产出无关。
...
第二个组成部分是利用你的社交媒体存在来扩大你的科学工作的受众。只有在第一步中成功开发了网络和社区，您才能这样做。然后，当您发布您自己的科学论文时，它们将被分享。
...
为了最有效地实现所有这些目标，您需要确定相关社区并建立一个关注您并将帮助分享您的想法和工作的个人网络。
...
最大的学术社区之一围绕 Twitter 发展起来，但一些科学家也出于专业目的使用 Facebook。如果您设置了一个 Twitter 帐户，那么您应该找到尽可能多的您所在专业领域的同事以及您所在领域的任何期刊。
...
如果您只是使用您的社交媒体帐户发布指向您发表的任何论文的链接，则很难培养大量追随者。通过不断发布博客文章等长篇原创内容也很难培养追随者。或者，您可以通过 (a) 有趣、(b) 有趣或 (c) 成为内容策展人来获得大量关注者。后一种方法对于刚接触社交媒体的人特别有用。只需关注您感兴趣的人和期刊，并分享您认为重要 / 有创意 / 令人兴奋的任何事情。
...
您创建的任何代码、出版物、数据或博客文章都可以从您的社交媒体帐户共享。这有助于提高您的知名度，因为人们会注意到您的出色工作。但是，如果您只发布自己的作品，那么除非您已经因其他原因出名，否则几乎不可能培养大量追随者。
...
科学家可以使用大量社交媒体平台。创造性地使用任何拥有大量用户的新社交媒体平台可以成为一种快速进入更多人意识的方式。话虽这么说，两个最大的科学家社区已经围绕两个最大的社交媒体平台组织起来。
...
*   [Twitter](https://twitter.com/) - is a platform where you can post short (less than 140 character) messages. This is a great platform for both discovering science and engaging in conversations about topics at a superficial level. It is not particularly useful for in depth scientific discussions.  
    Twitter - 是一个可以发布简短消息（少于 140 个字符）的平台。这是一个很好的平台，既可以发现科学，也可以从表面上讨论主题。它对于深入的科学讨论不是特别有用。
*   [Facebook](https://www.facebook.com/) - some scientists post longer form scientific discussions on Facebook, but the community there is somewhat less organized and people tend to use it less for professional reasons. However, sharing content on Facebook, particularly when it is of interest to a general audience, can lead to a broader engagement in your work.  
    Facebook——一些科学家在 Facebook 上发布较长形式的科学讨论，但那里的社区组织性稍差，出于专业原因，人们倾向于较少使用它。但是，在 Facebook 上分享内容，尤其是当普通观众感兴趣时，可以让更多人参与您的工作。
...
还有大量且越来越多的学术专用社交网络。在大多数情况下，这些社交网络并未被实践科学家广泛使用，因此并不代表您的时间得到了最佳利用。
...
您还可以考虑在 Vine 上播放短视频，在 Youtube 上播放更长的视频，在 Tumblr 或 Instagram 上播放更多图像密集型社交媒体，如果您的内容经常适合这些渠道。但他们往往拥有较小的科学家社区，来回交流的机会较少。
...
社交媒体可能会浪费时间，尤其是当您花费大量时间在您选择的平台上进行对话时。特别是生成长格式内容会占用大量时间。但是你不需要这样做来产生像样的追随者。找到合适的社区，然后在该社区内分享工作并添加简短的评论和想法通常可以帮助您培养大量的追随者，这对于其他原因可能很有用。

#### Add your own commentary 添加您自己的评论
...
一旦您习惯使用自己选择的社交媒体平台，您就可以开始与其他人进行对话，或者在分享其他人的作品时添加评论。这将增加人们对您的社交媒体帐户的兴趣，并帮助您培养关注者。这可以很简单，就像直接从您认为最重要的论文或帖子的文本中复制一行一样。
...
科学社交媒体的最大优势之一是它提供了公平的竞争环境。不要害怕与各级科学界的成员接触，从国家科学院的成员（如果他们在线！）一直到初级研究生。在科学会议和访问期间，结识不同的人确实对你有帮助。如果您花时间培养在线友谊，您经常会在参加的任何会议或会议上遇到一个 “熟悉的人”。

#### Include images when you can 尽可能包含图像
...
如果您从一篇论文中看到您认为特别引人注目的情节，请复制它并在您链接到该论文时发布 / 推文时附上它。在社交媒体上，图像通常比纯文本更容易被接受。
...
在社交媒体上要记住的一件事是意见的放大。有很多问题引起了极大的兴趣，并在多方面产生了非常强烈的意见。其中一些问题是常见的社会问题（例如，种族主义、女权主义、经济不平等），还有一些是科学特有的（例如，开放获取出版、开源开发）。如果您要开设一个社交媒体帐户来参与这些主题，那么您绝对应该这样做。如果您主要将您的帐户用于科学目的，您应该仔细考虑介入这些讨论的后果。社交媒体上的辩论非常激烈，您可能会发布您认为对这些主题之一相对切题或轻松的信息，并发现自己成为很多关注的焦点（正面和负面）。

Teaching in science 科学教学
------------------------
...
教学曾经是一项仅限于您当地机构的活动。教学的超地方性有一些影响。首先是你的科学声誉受你的教学影响较小，无论是好是坏。您现在可以使用 Internet 将您的教材提供给全世界，甚至可以将整个课程放到网上。这意味着您的教学可以在科学界获得良好、积极的声誉。这也意味着您在决定在线发布哪些教材时应该谨慎行事。
...
本章重点介绍面向互联网的教学，将其作为与世界共享教育材料的现代渠道。它与具体的教学法关系不大，而更多地与如何将课程传播给大量听众有关。
...
即使您不将教学材料组织成带有评估的课程，第一步也是将所有材料放在网上。如果您希望人们能够引用它们，您可以通过您的个人网站或 Figshare 分享它们。您使用的任何材料（包括讲义、代码、作业和测试）都可以帮助您的同事，对世界各地的学生有用，并提高您的声誉。

**Put teaching videos online 把教学视频放到网上**
...
如果你亲自教课，你可以通过屏幕抓取你的笔记本电脑或平板电脑。然后您可以将经过最少编辑的视频直接发布到 Youtube。包括指向您的课程材料的链接，这是将您的讲座传播给更广泛的听众的简单方法。

**(Optional) Build a MOOC （可选）构建 MOOC**
...
MOOC 是一个大规模的在线开放课程。有不同的平台可以提供这些课程，并附有评估和证书。如果您教授需求量大的主题类型（主要是白领职业），那么最大的 MOOC 平台可以显着增加您的材料所覆盖的受众。与将材料在线发布到 Youtube 相比，MOOC 需要付出更多的努力。

**(Optional) Answer questions on forums  
（可选）在论坛上回答问题**

There are now a wide range of forums for answering technical and scientific questions.  
现在有各种各样的论坛可以回答技术和科学问题。

*   [Quora](https://www.quora.com/) - includes curated answers, often from professionals, primarily to technical questions but also a wide range of questions of interest to academics.  
    Quora - 包括精选的答案，通常来自专业人士，主要是技术问题，但也包括学术界感兴趣的各种问题。
*   [Stackoverflow](http://stackoverflow.com/) - is a forum for posting answers to questions about computer programming  
    Stackoverflow - 是一个用于发布有关计算机编程问题的答案的论坛
*   [Reddit](https://www.reddit.com/) - is a general purpose site where you can find message boards (called subreddits) on a variety of scientific topics.  
    Reddit - 是一个通用站点，您可以在其中找到有关各种科学主题的留言板（称为 subreddits）。
...
您可以通过监控这些网站或其他特定领域的网站并提供问题的答案来提高您的教学声誉。如果你经常这样做，它可能会提高你在学术界内外的教学声誉。

### Teaching - what tools should I use?  
教学 - 我应该使用什么工具？

#### Tools for sharing lecture notes 分享讲义的工具

*   [Slideshare](http://www.slideshare.net/) and [Speakerdeck](https://speakerdeck.com/) can be used for sharing lecture slides and come with basic analytics.  
    Slideshare 和 Speakerdeck 可用于共享讲座幻灯片并提供基本分析。
*   [Figshare](https://figshare.com/) can be used to share data, code, and slides and gives DOIs which can be cited.  
    Figshare 可用于共享数据、代码和幻灯片，并提供可引用的 DOI。

#### Tools for sharing lecture videos 分享讲座视频的工具

*   [Youtube](https://www.youtube.com/) - is the dominant force and one of the easiest ways to share videos online. You can also develop a list of subscribers who will see your videos and potentially be directed back to your course content.  
    Youtube - 是主导力量，也是在线共享视频的最简单方式之一。您还可以制定一个订阅者列表，他们将看到您的视频并可能被引导回您的课程内容。
*   [Vimeo](https://vimeo.com/) - is an alternative platform to Youtube and is frequently used by artistic types. Particularly if you are making creative or artistic lecture videos this might be a good alternative.  
    Vimeo - 是 Youtube 的替代平台，经常被艺术类型使用。特别是如果您正在制作创意或艺术讲座视频，这可能是一个不错的选择。

#### Tools for sharing MOOCs 共享 MOOC 的工具

*   [Coursera](https://www.coursera.org/) - is the largest MOOC provider. It is a for-profit company and you can only teach a course on the platform if your university has an agreement with Coursera and the university approves.  
    Coursera - 是最大的 MOOC 提供商。它是一家营利性公司，只有在你的大学与 Coursera 达成协议并且大学批准的情况下，你才能在该平台上教授课程。
*   [Edx](https://www.edx.org/) - is another large MOOC provider. It is a non-profit, but again you can only teach a course on the main platform if your university has an agreement with Edx and the university approves.  
    Edx - 是另一家大型 MOOC 提供商。它是一个非营利组织，但如果你的大学与 Edx 达成协议并且大学批准，你只能在主平台上教授课程。
*   [Udemy](https://www.udemy.com/) - this is not a MOOC platform, but allows you to create complete courses including videos and quizzes and sell them directly to consumers. In this case you would not be directly affiliated with your university.  
    Udemy - 这不是 MOOC 平台，但允许您创建包括视频和测验在内的完整课程，并将它们直接出售给消费者。在这种情况下，您不会直接隶属于您的大学。
...
正如任何 Youtube 或 Vine 艺术家都可以告诉您的那样，互联网上的注意力持续时间非常短。如果可以的话，尽量将个别讲座的时长控制在 10 分钟以内是个好主意。您应该考虑将较长的材料分解成一系列视频。

#### Develop a thick skin 培养厚脸皮
...
请记住，当您在互联网上教学时，它就像互联网上的其他任何东西一样，会有大量的批评、一些非常有帮助的支持者和大多数沉默的人。教授一门成功的在线课程的一个重要组成部分，尤其是对大量听众来说，是在应对巨大的异质性反应时培养厚脸皮和冷静的举止。
...
许多大学对在线课程知识产权的思考还相对不成熟。一些大学会对您的讲座材料主张知识产权，而其他大学则不会。如果您打算使用像 Udemy 这样的平台，在这些平台上，讲师绕过大学基础设施直接向消费​​者提供课程，那么您应该仔细注意这一点。如果您的课程非常受欢迎或产生大量收入，这可能只会成为一个主要问题。
...
传统的图书出版模式包括让图书编辑相信您的想法不错，撰写一本书的草稿，对其进行审阅，然后最终将其出版。该系统还意味着最大份额的图书利润流向了出版商，而大多数消费者是图书馆。现代工具使出版书籍的速度更快，获得更广泛的反馈成为可能，同时还能保持更大的利润份额。

**Avoid traditional publishers 避开传统出版商**
...
传统出版流程缓慢，除非作者出名，否则不会为作者提供大量广告，也不会将大部分利润返还给作者。此外，除非它是绝对经典，否则你获得的学分数量是有限的。
...
现在有很多在线写书和分享草稿的平台。当你在写你的书时，或者当它是草稿时，在线分享它并从你未来的读者那里得到反馈。这将帮助您培养受众并为您提供强烈而即时的反馈。

**Sell your book on multiple online platforms  
在多个在线平台上销售您的图书**

Selling your book across multiple online platforms like [Amazon](http://www.amazon.com/) and [Leanpub](https://leanpub.com/) maximizes your audience. It also allows you to shop for the best royalty deals across platforms and drive your users to the platform that is best for you.  
在 Amazon 和 Leanpub 等多个在线平台上销售您的图书可以最大限度地扩大您的读者群。它还允许您购买跨平台的最佳版税交易，并将您的用户带到最适合您的平台。

### Books - what tools should I use?  
书籍 - 我应该使用什么工具？

#### Writing and selling 写作和销售

There are a number of tools for developing and selling books online including:  
有许多在线开发和销售图书的工具，包括：

*   [Leanpub](https://leanpub.com/) - a good platform where you can edit books with Markdown. You get 90% of royalties after a processing fee. This is one of the best royalty rates available. You can also publish books in draft mode and collect a mailing list ot let people know when you have a new book.  
    Leanpub - 一个很好的平台，您可以在其中使用 Markdown 编辑书籍。收取手续费后，您将获得 90% 的版税。这是可用的最佳版税率之一。您还可以以草稿模式发布书籍并收集邮件列表，以便在您有新书时让人们知道。
*   [Gitbook](https://www.gitbook.com/) - has a slightly nicer interface for writing books and you can export them to all formats so you can sell them across multiple platforms.  
    Gitbook - 有一个稍微好一点的写书界面，你可以将它们导出为所有格式，这样你就可以在多个平台上销售它们。
*   [Amazon Kindle Direct Publishing](https://kdp.amazon.com/) - probably has the largest audience but not a great royalty rate, as low as 30% for some books sold in some countries. This is your only chance to make a million online publishing - realistically that isn’t going to happen though.  
    Amazon Kindle Direct Publishing——可能拥有最多的读者，但版税率并不高，在某些国家 / 地区销售的某些书籍的版税率低至 30%。这是您获得一百万在线发布的唯一机会 - 实际上这不会发生。
...
一旦你在网上写了你的书，你就需要做你自己的广告。宣传您以这种方式编写的书籍的一些有用方法包括您自己的社交媒体、博客和在线课程。在社交媒体上联系其他博主或您的朋友，看看他们是否会帮助您推广您的作品也很有用。在这方面稍作努力确实可以扩大您的受众群体。

#### Collecting emails 收集电子邮件
...
如果你使用像 Leanpub 这样的平台，读者可以注册以在你有新书时收到提醒。这种邮件列表可以稳定地建立起来，这样你就会有读者阅读你决定写的任何新书。
...
与所有面向互联网的活动一样，人们的注意力通常相对较短。保持你的书简短和重点突出可以避免没有你的书的纸质副本阅读的用户的疲劳。

#### Physical copies with on-demand publishing  
按需出版的物理副本

Here we have focused on internet and web based publishing as the primary outlet for modern scientific books. If you want to get print copies made the ideal situation is to use an on-demand publisher like [Lulu](https://www.lulu.com/). You can upload your books in a slightly modified format from the version you created electronically and then set a price. The books will be printed as people pay for them, so there are no up front printing costs.  
在这里，我们专注于将互联网和基于网络的出版作为现代科学书籍的主要渠道。如果您想制作印刷品，理想的情况是使用像 Lulu 这样的按需出版商。您可以从您创建的电子版本中以稍微修改过的格式上传您的图书，然后设定价格。这些书将在人们支付费用后印刷，因此没有前期印刷成本。
...
管理科学过程的关键组成部分之一是与您的内部科学团队进行沟通。科学的困难在于，这个团队总是在变化，而且几乎总是同时处理多个项目和重叠的个人。为了有效地沟通，您需要一种方式来与特定项目的人员进行沟通，记录正在发生的事情的历史记录并记录进度，以及让新团队成员快速跟上进度的方式。
...
当您有一个科学团队需要管理给定项目时，以小组形式组织交流会很有帮助。电子邮件是一种选择，但人们需要能够进入和离开讨论，因此最好使用一款可以管理群组的软件。

### Internal communication - what tools should I use?  
内部沟通——我应该使用什么工具？

There are a number of tools that can be used starting with simple message boards like [Google Groups](https://groups.google.com/forum/#!overview). Lately there have been new software platforms developed for team communication that seem particularly useful for scientific groups.  
有许多工具可以从简单的留言板开始使用，例如 Google 网上论坛。最近出现了为团队交流开发的新软件平台，似乎对科学团体特别有用。

*   [Slack](https://slack.com/) - is a group messaging app that can be used to manage groups of individuals and keep track of conversations. It does cost money if you want to keep track of old conversations. But it includes integrations for lots of other tools like Github, Google Docs, and Dropbox that make it very useful.  
    Slack - 是一款群发消息应用程序，可用于管理个人群组并跟踪对话。如果您想跟踪旧的对话，确实要花钱。但它包括许多其他工具的集成，例如 Github、Google Docs 和 Dropbox，这使得它非常有用。
*   [Hipchat](https://hipchat.com/)- is an alternative to Slack that has many of the same features. It has a slightly different pricing system and isn’t as widely used.  
    Hipchat - 是 Slack 的替代品，具有许多相同的功能。它的定价系统略有不同，并没有被广泛使用。

There are a number of open-source and self-hosted alternatives that are being developed to these main two platforms including things like [Mattermost](http://www.mattermost.org/). But for the most part, unless you want to manage a lot of the issues with a communication platform Slack or Hipchat are the best options.  
正在为这两个主要平台开发许多开源和自托管替代方案，包括 Mattermost 等。但在大多数情况下，除非你想管理很多问题，否则使用通信平台 Slack 或 Hipchat 是最好的选择。
...
当您同时处理多个项目时，针对您正在处理的每个项目进行讨论会很有用。这些讨论应该向您认为合适的尽可能广泛的听众开放，但应该受到监控以确保它们不会偏离主题。当一个项目完成时，理想的情况是将这个讨论移到一个存档的部分，在那里它可以随时被引用，但不会弄乱主要的讨论板。

#### Summarize at regular intervals 定期总结
...
使用任何群组通信工具时，都倾向于进行长时间不间断的讨论。一直直接关注讨论的人追起来没有问题，但新成员或不定期关注的人会被排除在外。对于每次讨论，有一个运行中的 “最新摘要” 文档是很有用的，该文档可以用讨论中提出的任何关键点进行更新，并且可以快速用于让人们了解最新情况。
...
要记住的一件事是，如果您使用 Slack 和 Hipchat，则需要随着时间的推移对成员进行管理。当个人出现时，他们应该获得一个帐户，当他们离开时，他们应该被淘汰。这样您就可以 (a) 降低成本，因为大部分付款是由用户 / 使用支付的，并且 (b) 确保您的团队内部的信息不会与更广泛的社区共享。

#### Include a thread for literature curation  
包括用于文学策展的线程

One of the best ways for people to discover what is going on with a scientific group is to know the relevant scientific literature. The best way to do that is to have people post internally on a seperate channel or group any paper they think is interesting along with any commentary they think is appropriate. This group can often be the easiest way to get a student up to speed on a group’s scientific thinking.  
人们发现科学团体正在发生的事情的最好方法之一是了解相关的科学文献。做到这一点的最佳方法是让人们在单独的频道或群组中发布他们认为有趣的任何论文以及他们认为合适的任何评论。这个小组通常是让学生跟上小组科学思维的最简单方法。

#### Generate onboarding documents 生成入职文件
...
科学团体的人员流动率总是很高。学生和博士后在开始新的学术生涯时会定期更换。为避免不必要的生产力中断，拥有一些可用于让学生加快速度的入职文件很有用。一些有用的入职文件包括以下内容。
...
无论他们是实验实验室、如何订购新试剂、如何获得计算机或如何登录服务器，将小组使用的工具的所有相关后勤细节存储在一个地方都是很有用的。
...
为一组执行的最常见的实验或分析收集最新的协议是很有用的。这些既可以作为学习练习，也可以让新成员从一开始就准备好帮助完成项目。

Scientific talks 科学讲座
---------------------
...
当你刚开始的时候，你应该接受几乎所有的机会来谈论你的研究。按照重要性的大致顺序，在您的职业生涯早期进行的谈话的价值是：

1.  To meet people 认识人
2.  To make people excited about your ideas/results  
    让人们对您的想法 / 结果感到兴奋
3.  To make people understand your ideas/results  
    让人们理解你的想法 / 结果
4.  To practice speaking 练习口语
...
后来原因不断演变，尽管不像你想象的那么多。主要的变化是 4 更多地演变为 “向人们展示你是一个很好的演讲者”。
...
第 1 点的重要性怎么强调都不为过。您进行演讲的主要原因是让人们了解您。受到尊重绝对不是学术界的目标。然而，知名度和知名度可以让你的工作中的很多部分变得更容易。因此，首先要确保在演讲之前、之后和期间不要忘记与人交谈。
...
*   If you entertain people **they might go read your paper/use your code/download your data** which is the best way to achieve goal 3.  
    如果你招待人们，他们可能会去阅读你的论文 / 使用你的代码 / 下载你的数据，这是实现目标 3 的最佳方式。
...
*   **Job talk**: 工作谈话：
    ...
    *   **What to talk about**: Brief overview of who you are, intro to your (single) problem, how you solved it, results, summary of what you have done/plan to do.  
        谈论什么：简要概述你是谁，介绍你的（单一）问题，你是如何解决它的，结果，你已经完成 / 计划要做的事情的总结。

### Scientific talks - what tools should I use?  
科学讲座——我应该使用什么工具？

#### Making the presentation 进行演示

In general any of the standard presentation creation tools will work including [Powerpoint](https://office.live.com/start/PowerPoint.aspx), [Google Slides](https://www.google.com/slides/about/), or [Keynote](http://www.apple.com/mac/keynote/). Avoid using software like Latex where you have to adjust the figures through text encoding. When making presentations you want to use software that makes it easy to move and adjust figures.  
一般来说，任何标准的演示文稿创建工具都可以使用，包括 Powerpoint、Google Slides 或 Keynote。避免使用像 Latex 这样的软件，你必须通过文本编码来调整数字。在进行演示时，您希望使用可以轻松移动和调整图形的软件。
...
如果您的演讲中有网络链接，您需要将其发布到网上。人们不可能记下您演讲中的任何链接。进行演讲的两个好地方是 https://speakerdeck.com/ 或 http://www.slideshare.net/。然后从一个单一的、令人难忘的站点链接到所有演讲，您可以在演讲结束时向人们展示。我所有的演讲都在 http://jtleek.com/talks/。如果与 Leek 小组相关，欢迎您在演讲中发送拉​​取请求，或者您可以将它们放在您自己的简短网站链接中。
...
这些天我个人更喜欢 Slideshare，因为幻灯片在手机和 iPad 上更容易查看，这是人们最有可能阅读你的演讲的地方。

### Scientific talks - further tips and issues  
科学讲座 - 更多提示和问题

#### Structure of your talk 你演讲的结构
...
演讲中最大的陷阱是假设其他人会因为你跟随演讲而跟随你。一般来说，最好假设你的听众知道的比你想象的要少。人们喜欢感觉聪明。我很少听到有人抱怨他们的解释过于简单，但经常听到有人抱怨他们迷路了。话虽如此，这里有一些结构性技巧。你的旅费可能会改变。

*   **Always lead with a brief, understandable to everyone statement of your problem**.  
    始终以简短的、每个人都可以理解的方式陈述您的问题。
*   Explain the data and measurement technology before you explain the features you will use  
    在解释您将使用的功能之前先解释数据和测量技术
...
最后一点尤为重要。通常在结果部分，人们会变得有点坐立不安。因此，一组完全不同的结果背后几乎没有什么故事会让人们发疯。确保预先解释结果的去向（例如 “结果将显示我们的方法是最快 / 最准确 / 有史以来最好的”），然后确保您的结果根据它们的意义划分为多个部分并组织起来就像你在讲故事一样。

#### Style of your talk 你说话的风格
...
这里只有一些硬性规定。我真的很喜欢 Zach Holman 的风格指南 speaking.io 和他的实际演讲。我的建议是选择一种模板 / 样式并使用它连续进行几次讨论，以避免代价高昂的开销。看看你喜欢什么，不喜欢什么，然后编辑。这是唯一的硬性规定。

*   Title slide must have a contactable form of you (twitter handle, email address, etc.)  
    标题幻灯片必须有您的联系方式（推特句柄、电子邮件地址等）
*   Fonts can never be too big. Go huge. Small fonts will be met with anger.  
    字体永远不能太大。变得巨大。小字体会被激怒。
...
*   Explain what trends the audience should look for (e.g. “curves that go straight up at zero and across the top of the plot are best”)  
    解释观众应该寻找什么趋势（例如 “在零处直线上升并穿过图顶部的曲线是最好的”）

#### About equations in your talks 关于你演讲中的方程式
...
如果你正在进行实证演讲，你可能会有一些方程式。那没问题。但是你展示它们的方式对于进行可理解的演讲至关重要。这里有几个要点：
...
在进行工作谈话时，除了我们上面谈到的四个主要目标之外，你还有一份额外的工作。目标是向一群（大多数）不知道你是谁的人展示你完整的职业形象。你应该根据你申请工作的地方稍微调整一下，但不要试图假装你不是。你可以在理论部分展示更多的理论，在应用部分展示更多的结果，但如果你不是，就不要声称你一直在证明定理。
...
你应该在谈话的开头和结尾都包括你所做的所有工作和计划​​工作的简短摘要，这样他们就能全面了解你是谁。但是在演讲时只谈论一个特定的项目。没有什么比随着时间的流逝或没有完整的演讲更能影响您找到工作的机会了。
...
传统上，人们通过证明他们可以证明非常难的定理来证明 #2。在某些地方，这仍然是一件非常好的事情。其他可能让你与众不同的东西是编写令人惊叹的 R 包并得到使用的能力，分析其他人无法分析的海量数据集的能力，教授学生想上但部门没有的课程的能力，或关于全新研究领域的想法（有一些数据支持）。
...
在进行工作谈话之前，请四处询问并了解人们听说过的有关您申请的地方的各种事情，这样您就可以明智地选择展示方式 / 展示内容。

#### Answering hard questions 回答难题
...
在你的演讲中，你不可避免地会遇到尖锐的问题。最重要的一点是不要恐慌，不要采取防御措施。最好只是说我不知道​​，然后生气。当你被问到一个非常困难的问题时，你应该：
...
*   Say _I don’t know_ if you don’t know. If you say I don’t know, then you can give your best guess and explain it is a guess.  
    说我不知道​​，如果你不知道。如果你说我不知道​​，那么你可以给出你最好的猜测并解释这是一个猜测。
...
关键是要区分你给出的是哪种回应。您是否在因为您实际调查过而知道答案的情况下做出回应？您是否在完全不知道的地方给出了完整的猜测？或者，更有可能的是，您介于两者之间吗？
...
最重要的是，不要感到尴尬！每个人都会遇到棘手的问题，如果您有礼貌，请说明您对自己的答案有多确定，并尽力而为，这不会成为问题。
...
几乎在任何你演讲的地方都会有一个情绪低落 / 紧张 / 咄咄逼人的人。不要陷入以攻击性回报的诱惑。像其他人一样礼貌地回答他们的第一个问题。如果他们一直问非常激进的 / 很多问题，你有权说：“你提出了很多好的问题，我很乐意在演讲后更深入地与你讨论，但为了利益有时间我会继续我的下一部分”。让事情继续进行并按时完成是可以的。

#### Finishing on time 按时完成
...
做吧。人们会因此而爱你。如果您是会议中的最后一位发言者，而其他人发言时间较长，请适应并缩短发言时间。通过让听众开心而获得的价值 >>>> 您本可以解释的额外 5 分钟细节。

_Note_: Parts of this chapter appeared in the [Leek group guide to giving talks](https://github.com/jtleek/talkguide)  
注：本章部分内容出现在 Leek group guide to giving talks

Reading scientific papers 阅读科学论文
--------------------------------
...
学术论文仍然是向世界传播新知识的主要方式。还有其他方法，使用代码、博客或推特。但学术论文仍然是黄金标准，也是报告绝大多数新科学发现的地方。
...
好吧，这在很大程度上取决于您对什么感兴趣。根据您要寻找的内容，期刊有几大类。
...
*   **Science magazines**: Journals like [Nature](http://www.nature.com/index.html), [PNAS](http://www.pnas.org/), and [Science](http://www.sciencemag.org/) publish papers that are supposed to be “breakthroughs” of interest to a “general audience”. This means that the papers tend to be written at a slightly less technical level and so are often more readable to people outside of a particular field. It also means that a lot of the most important papers get published in these journals. The flip side is that people often stretch hard in the interpretation of their data to make it seem like a “breakthrough” and get it into one of these journals. This means that the rate that papers are retracted is also [very high](http://iai.asm.org/content/79/10/3855.long) in these journals. So read what you see there with a healthy grain of salt.  
    科学杂志：《自然》、《美国国家科学院院刊》和《科学》等期刊发表的论文应该是 “普通读者” 感兴趣的“突破”。这意味着论文的技术水平往往略低，因此对于特定领域以外的人来说通常更具可读性。这也意味着许多最重要的论文都发表在这些期刊上。不利的一面是，人们经常在解释他们的数据时竭尽全力，使它看起来像是一个“突破”，并将其纳入这些期刊之一。这意味着这些期刊的论文被撤稿率也很高。因此，请谨慎阅读您在那里看到的内容。
*   **Health magazines**: In the health sciences there is a similar set of journals like the [New England Journal of Medicine](http://www.nejm.org/) and the [Journal of the American Medical Association](http://jama.jamanetwork.com/journal.aspx). They have the same benefits and caveats as the Science magazines, but with a more health flavored bent.  
    健康杂志：在健康科学领域，有一套类似的期刊，如新英格兰医学杂志和美国医学会杂志。它们具有与科学杂志相同的好处和注意事项，但更偏向于健康。
*   **Mega journals**: Journals like [PLoS One](http://www.plosone.org/) and [Peerj](https://peerj.com/) also publish in a wide range of areas and a ton of papers. The review criteria here is that it must be “correct” but not necessarily a “breathrough”. So the heterogeneity in the papers is high. If a paper seems too good to be true, again it is worth taking with a grain of salt.  
    大型期刊：像 PLoS One 和 Peerj 这样的期刊也发表了广泛的领域和大量论文。这里的审查标准是必须 “正确” 但不一定是“突破”。所以论文的异质性很高。如果一篇论文看起来好得令人难以置信，那么再次值得怀疑。
*   **Field-specific Scientific journals**: Most scientific journals are not megajournals or magazines. These journals tend to be very field specific and tend to be much heavier on the details. This is where most science is published. The papers tend to be less focused on “breakthroughs” but are also less consistently risky to trust in these journals. In my area the journals might be something like [Biostatistics](http://biostatistics.oxfordjournals.org/) or [Biometrics](http://www.biometrics.tibs.org/).  
    特定领域的科学期刊：大多数科学期刊都不是巨型期刊或杂志。这些期刊往往是非常特定领域的，并且往往在细节上要重得多。这是大多数科学发表的地方。这些论文往往不太关注 “突破”，但信任这些期刊的风险也较低。在我所在的领域，期刊可能类似于 Biostatistics 或 Biometrics。
*   **Conference papers**: In some fields, like computer science, people tend to publish in short, peer reviewed conference papers. These papers tend to be quite technical - conferences like [NIPS](https://nips.cc/) publish similar papers to very technical journals in other fields. Conference papers tend to be lighter on the detail and tend not to come with software/code so they can be a little harder to read and a little harder to use, but they are often talking about the very latest, coolest ideas in their subfield.  
    会议论文：在某些领域，如计算机科学，人们倾向于发表简短的、经过同行评审的会议论文。这些论文往往是非常技术性的——像 NIPS 这样的会议向其他领域的非常技术性的期刊发表类似的论文。会议论文在细节上往往更轻松，而且往往不附带软件 / 代码，因此它们可能更难阅读和更难使用，但他们经常谈论其子领域中最新、最酷的想法。
...
*   You can also read papers in biomedical sciences on aggregator sites like [Pubmed Central](http://www.ncbi.nlm.nih.gov/pmc/)  
    您还可以在 Pubmed Central 等聚合网站上阅读生物医学科学论文
...
期刊网站的一个问题尤其是许多论文都在付费专区后面——你必须付费才能阅读它们（见下一节）。您越来越多地可以在称为预印本服务器的东西上找到最新的论文。这些论文尚未经过同行评审，但其中很大一部分最终会出现在经过同行评审的期刊中。这些论文的好处在于它们通常是最新的研究成果并且可以免费阅读。两个好的预印本服务器是 bioRxiv 和 arXiv。

#### Open access and #icanhazpdf 开放获取和 #icanhazpdf
...
如果你遇到一篇需要花钱的论文，首先要做的是查看作者是否发表了预印本，然后查看他们的网站。如果你仍然不走运，你可以直接给作者发邮件索要他们论文的副本（他们通常很乐意帮忙）。
...
一种更现代的方法已经出现，称为 _#icanhazpdf_ ，这是一种你可以众包你无法阅读的论文的 pdf 的方法。如果您有 Twitter 帐户，请发布该论文的链接、#1# 标签和您的电子邮件地址，通常会有人愿意找到并向您发送该论文的副本。获得副本后，删除你的推文，因为这种方法在技术上是违规的，可能会给你带来麻烦。如果您不对大量论文一遍又一遍地这样做，大多数期刊不会在意，但请注意，当期刊的利益受到威胁时，它们可能会变得令人讨厌 / 律师。

#### Scihub

Currently there are a large number of papers behind paywalls. This is the subject of [ongoing debate](http://www.nytimes.com/2016/03/13/opinion/sunday/should-all-research-papers-be-free.html?_r=0) among scientists, the press, and the public. Currently there is one tool that allows you to access all papers directly through the web. This tool [sci-hub.io](https://sci-hub.io/) allows you to insert the link for any paper (closed or open access) and returns a pdf. It is important to note that sci-hub is of questionable legality, but for the moment represents the easiest way to share closed access papers for which there is no preprint available. Ideally in the future, a more acceptable funding model will be created with less legal constraints.  
目前，付费专区背后有大量论文。这是科学家、媒体和公众之间持续争论的主题。目前有一种工具可以让您直接通过网络访问所有论文。此工具 sci-hub.io 允许您插入任何论文（封闭或开放访问）的链接并返回 pdf。重要的是要注意 sci-hub 的合法性值得怀疑，但目前它代表了共享没有预印本的封闭访问论文的最简单方法。理想情况下，在未来，将创建一个法律约束更少的更可接受的融资模式。

### Reading scientific papers - further tips and tricks  
阅读科学论文——进一步的提示和技巧

#### How much should you read? 你应该读多少？
...
学术论文一直在出现。每年出版数以千计，其中包括任何特定领域的数百本。除非你全神贯注地阅读学术论文，否则你将无法跟上所有的进度。我相信你应该阅读你觉得有趣的论文的想法。科学很棒，如果可以避免的话，你不应该把时间浪费在无聊的部分上。
...
一般来说，有两种主要的方法可以找到我喜欢的论文。我过去的做法是使用我喜欢的期刊的 RSS 提要设置一个聚合器，然后我使用以下（近似）阅读率来阅读部分论文。

*   100% - read the title 100% - 阅读标题
*   20-50% - read the abstract 20-50% - 阅读摘要
...
我这样做的新方法是关注 bioRxiv 和一群在 Twitter 上有相似兴趣的人。我将上述百分比用于聚合器发布的推文，如果我看到一篇由我信任的 2-3 人发布的论文，我通常最终会阅读该论文。
...
不同的人会有不同的策略。首先，我阅读了标题和摘要，以了解 (a) 作者认为这篇论文有趣的原因以及 (b) 论文的主要结果是什么。我这样做是为了看看我是否认为这篇论文值得花时间更深入地阅读。我根本不从这些组成部分判断质量，只是这篇论文是否有趣。
...
如果我根据标题 / 摘要认为这篇论文很有趣，那么我接下来要做的就是查看图表和图表说明。正如我在我的论文写作指南中提到的那样，图表应该讲述一个连贯的故事，并且应该有图表说明来解释正在发生的事情。
...
希望您正在阅读的论文中有这样易于阅读的图表。我通常在寻找作者试图讲述的 “故事”。对于统计或计算论文，我也在寻找与以前方法的比较以及这种方法如何叠加。
...
我通常跳过介绍。这通常是摘要的扩展版本，并且通常包含更多关于特定结果有多棒的观点而不是事实。
...
我必须遵守这条规则的一个例外是，如果我不太了解科学领域。然后我阅读了介绍中回顾该领域以前工作的部分，如果我不明白什么，我会从介绍中找出参考文献并通读这些以 “加快速度”。
...
如果我决定仔细阅读一篇论文，我会花大部分时间阅读方法和补充材料。这就是大多数真正的 “科学” 所在。它告诉你他们是如何进行实验的，他们是如何分析数据的，以及他们是如何支持他们的结论的。当我在高层次阅读方法部分时，我正在寻找一些东西，包括：
...
不幸的是，在那之后你必须了解该地区才能更批判性地判断他们所做的事情是否好。这与实践或某个领域的专业知识有关，不能很容易地概括为简洁的指南。

#### Reading papers - the results 阅读论文——结果
...
我发现如果作者完成了他们的工作并使他们的数字清楚地讲述了故事，那么我通常会花更少的时间阅读结果部分。关键结果通常在数字中，但我仍然会浏览这一部分，看看是否有任何我在阅读数字时遗漏的主张 / 想法。总的来说，我非常仔细地将这一部分与方法进行比较，以确保结果与他们在方法部分所说的相比似乎是合理的。

#### Reading papers - Conclusions 阅读论文 - 结论
...
就像介绍一样，我经常跳过结论。它通常只是对论文其余部分发生的事情的回顾，并对结果如何适合更广泛的方案进行一些猜测。
...
要记住的一件事是，科学通常是缓慢、稳定和渐进的。但是科学家们面临着很大的压力来提出 “突破”（有时称为“我在这里有一个大突破” 的谬论）。在阅读一篇论文时，如果作者声称他们已经治愈了癌症、在火星上发现了生命，或者统一相对论和量子理论，那么你应该假设他们充满了这些，除非有确凿的证明。

#### Explain it to someone else 给别人解释
...
阅读学术论文是获取知识的好方法。但总的来说，在我向其他人解释论文之前，我不觉得自己理解论文中发生的事情。因此，我尝试与其他人讨论我认为非常重要的论文。做到这一点的最好方法是在期刊俱乐部或其他论坛中，您可以在其中张贴论文中的数据并尝试自己解释发生了什么。
...
很多论文我觉得很有趣，其他人也觉得很有趣。了解更多关于一项科学研究的一个好方法是看看它是否已经在博客上讨论过。要记住的一件事是博客通常有一个议程，因此您也应该带着强烈的怀疑态度阅读这些帖子。尽管如此，它们仍然可以为您阅读的论文提供有用的观点。

_Note_: Part of this chapter appeared in the [Leek group guide to reading a scientific paper](https://github.com/jtleek/readingpapers).  
注：本章部分内容出现在 Leek group guide to reading a scientific paper 中。
...
在学术界，出版物仍然是获得强大和持久尊重的唯一真正途径。因此，即使您在社交媒体上拥有大量追随者、共享大量数据集并疯狂地发布发表评论，如果不在优秀期刊上发表文章，您也很难在传统学术界引起轰动。在某些方面，这是非常令人沮丧的，但在其他方面，它强调了在科学中仔细、彻底工作的重要性。

__Magnify your publications with modern scientific principles __  
__用现代科学原理放大你的出版物__
...
做我在本书中谈到的许多事情的一个原因是扩大你在科学界的影响力。如果您定期在学术期刊上发表文章，您可以使用社交媒体让更多人阅读您的作品，使用博客来讨论论文中不适合的细节，并分享您的代码和数据来让更多人引用您的论文。至少目前，这种间接学分是你在传统学术界所能期待的最多的

**Quantify everything 量化一切**
...
您可以用来开始为您的社交媒体、编码和数据共享技能构建案例的一件事是量化所有可能的事情。阅读您的博客的人数、您的数据的下载次数以及使用您的代码的人数在建立学分案例时都很重要。
...
为现代科学活动争分夺秒的主要方法是 (a) 量化它们，以及 (b) 将它们置于更广泛的学术轨迹背景中。第一步是收集数据。这些站点可用于收集有关您的研究成果的有价值信息。

*   [Google Analytics](https://www.google.com/analytics/) - for quantifying visitors to websites that you have.  
    Google Analytics - 用于量化您拥有的网站的访问者。
*   [Youtube Analytics](https://www.youtube.com/analytics) - for quantifying numbers of views of your videos  
    Youtube Analytics - 用于量化视频的观看次数
*   [Github stars](https://help.github.com/articles/about-stars/) and [Github forks](https://help.github.com/articles/fork-a-repo/) as metrics for code use  
    Github stars 和 Github forks 作为代码使用的指标
*   Download statistics from specific curated repositories like on [Bioconductor](https://www.bioconductor.org/packages/stats/) for quantifying code use  
    从特定的精选存储库（如 Bioconductor）下载统计数据以量化代码使用
*   Number of followers on various social media sites  
    各种社交媒体网站上的关注者数量
*   [Alt metrics](https://www.altmetric.com/) - for quantifying social media interest in your research outputs  
    Alt 指标 - 用于量化社交媒体对您的研究成果的兴趣
*   [Publons](https://publons.com/) - for quantifying your refereeing output  
    Publons - 用于量化您的裁判输出
*   [Google Scholar](https://scholar.google.com/) - for quantifying citations to your academic work  
    Google Scholar - 用于量化对您的学术作品的引用
*   Number of views and downloads to your Figshare repositories for quantifying the impact of your data sharing.  
    您的 Figshare 存储库的查看次数和下载次数，用于量化数据共享的影响。

Once the data has been collected you need to try to put it in context. This is an excellent [presentation](https://github.com/jennybc/2015-08_bryan-jsm-stat-data-sci-talk/blob/master/2015-10_waller-data-science-promotion-tenure.pdf) by Lance Waller on how to put your achievements in context. You can also see his talk in this online conference on the [Statistics identity crisis](https://www.youtube.com/watch?v=JLs01Z5baSU&feature=youtu.be).  
收集数据后，您需要尝试将其置于上下文中。这是兰斯 · 沃勒 (Lance Waller) 关于如何将您的成就置于背景中的精彩演讲。你也可以在这个关于统计身份危机的在线会议上看到他的演讲。
...
直接在简历的相关部分中包括引用、下载和其他指标，以便于访问。考虑列出数字本身和您可以提供的任何背景信息（百分位数、奖项、认可度），以便人们可以轻松理解您提供的指标。在某些情况下，您的简历格式将由您的学校决定。如果这是真的，那么您应该考虑将所有指标汇编成一个单独的文档，概述您的整体影响。
...
围绕现代科学方法的大部分精力都集中在更多初级科学家身上。但是更多的资深科学家做出了大量的重要决定（教师聘用、资助、教师晋升）。一个非常好的主意是由资深科学家执行您的计划并获得他们的反馈。他们可以帮助您了解您的研究概况中哪些组成部分会以积极的方式脱颖而出，哪些并不重要。理想情况下，这将是一位导师，他甚至可能不同意你的方法，但不会因为你做了不同的事情而严厉地评判你。我很幸运有这样的导师。
...
人们从事科学的最常见原因是无私的。他们小时候就喜欢恐龙和宇宙飞船，而且这种爱好从未消失。在某种程度上，这是我如此热爱这个领域的原因之一，在这个领域，如果你能克服所有困难的部分，就能真正不断地为你每天的工作引入奇迹。
...
有时我觉得这种利他主义会带来负面影响。例如，我认为学术界对职业规划和发展方面的重视程度较低。我不认为这是恶意的，但我确实认为有时人们认为科学的职业部分是不体面的。但是，如果您有任何工作希望人们付钱让您去做，那么该工作的某些部分将以职业为导向。所以如果你想成为一名专业的科学家，光有才华和擅长科学是不够的。您还需要关注并仔细规划您的职业轨迹。
...
我的一位同事 Ben Langmead 在 Github 上为他的博士后创建了一个非常好的指南，用于思考和规划博士后的职业生涯。我觉得这是个好主意，我立即修改了它，并让我所有的研究生和博士后都填写了它。它有点长，所以如果他们没有完成它也不会受到惩罚，但我认为它是一个非常有用的工具，可以用来思考如何制定科学职业生涯的战略。我认为我们对研究生院和博士后的职业方面越具体，包括对所有可用的现实选择诚实，我们的学生就越能为在市场上取得成功做好准备。

**Have a career plan: 制定职业规划：**
...
我知道你想成为一名科学家，因为你热爱科学。这就是为什么我们都想成为科学家。但如果你想成为一名专业科学家并获得报酬，你还需要一个职业发展计划。从事科学事业可以带来令人难以置信的回报——教学生、做您认为重要的研究以及将这些想法传达给广大听众都很棒。还有一些事情很难。我认为不必要地困难的一件事是职业规划。如果您使用本指南，它可以帮助您理解作为我小组中的研究生或博士后开始的数据科学职业所带来的机遇和挑战。

**Figure out what you want to do:  
弄清楚你想做什么：**

One of the hardest parts of planning as a graduate student or postdoc is figuring out what you want to do next. Here are some relatively rough guidelines of the types of jobs that you could go after. These are by necessity very brief. Rafa’s post on [hard/soft money jobs](http://simplystatistics.org/2011/12/19/on-hard-and-soft-money/) and my adendum on [liberal arts colleges](http://simplystatistics.org/2011/09/15/another-academic-job-market-option-liberal-arts/) as well as this excellent post on [academia versus industry for a junior person](http://www.pgbovine.net/academia-industry-junior-employee.htm) are great places to start reading more.  
作为研究生或博士后，计划中最困难的部分之一就是弄清楚你下一步想做什么。以下是您可以从事的工作类型的一些相对粗略的指南。这些必然非常简短。拉法关于硬 / 软货币工作的帖子和我关于文理学院的附录以及这篇关于学术界与工业界的优秀帖子都是为初级人员开始阅读更多内容的好地方。
...
*   **Tenure track faculty/principal investigator research school**: You will primarily be writing lots of papers and to a greater or lesser extent (a) teaching, (b) writing grants, and (c) advising students. This job has some pressure associated with it because you are usually expected to get grants/write methods papers where you are in charge, but in some ways has the most freedom if you are successful.  
    Tenure track faculty/principal investigator research school：你将主要写很多论文，并在或多或少的程度上 (a) 教学，(b) 写作资助，和 (c) 为学生提供建议。这项工作有一些与之相关的压力，因为通常期望您在您负责的地方获得资助 / 撰写方法论文，但如果您成功，在某些方面拥有最大的自由。
*   **Tenure track liberal arts college professor**: You will write fewer papers and do more teaching. The grant pressure is less but is steadily growing. This is a good option if you are really into teaching and engaging with undergrad students.  
    Tenure track 文理学院教授：你会写更少的论文，多做教学。拨款压力较小，但正在稳步增长。如果您真的热衷于教学并与本科生互动，这是一个不错的选择。
*   **Research track faculty**: This job varies a ton by institution. It can be anything from a heavy teaching load to pure research and everything in between. Some places tend to treat these faculty really well. But that isn’t universally true. These jobs tend to require you to get fewer individual grants/write fewer methods papers but that isn’t a rule either.  
    研究轨道教师：这项工作因机构而异。它可以是从沉重的教学负担到纯研究以及介于两者之间的一切。有些地方往往会很好地对待这些教师。但这并非普遍如此。这些工作往往要求你获得更少的个人资助 / 写更少的方法论文，但这也不是规则。
*   **Industry**: I have less experience on this side so I’m hesitant to make too many generalizations. That being said I think there is everything on this side from pretty research-like jobs, to engineering style jobs, to very business oriented jobs. If you are into this area your duties will depend a lot on your exact job.  
    行业：我在这方面的经验较少，所以我不愿做太多概括。话虽如此，我认为这方面的工作应有尽有，从类似研究的工作到工程风格的工作，再到非常面向商业的工作。如果你在这个领域，你的职责将在很大程度上取决于你的具体工作。
...
本文档的其余部分是供您填写并保持更新的工作表。用蓝色写下你的答案，使它们脱颖而出。

_What’s your goal? 你的目标是什么？_

*   Where will you go to find relevant job listings? Hints: CRA, Science/Nature jobs,….  
    您会去哪里寻找相关的职位列表？提示：CRA，科学 / 自然工作，......
*   At a high level, academic jobs are a mix of research and teaching. A standard research-oriented “hard money” position is one where 75% of your salary is supported (in return for your teaching and administrative efforts), the other 25% you cover from grants, and you typically teach 2-3 courses a year. A “soft money” position is one where the majority (perhaps all) of your salary you cover from grants. You will usually teach 0 courses a year in this system; maybe you have to give some guest lectures. I don’t know as much about pure teaching positions, but I think they tend to be 100% supported but you teach upwards of 5-6 courses a year. I know less about industry jobs, but the skills they value tend to be (a) software engineering, (b) really applied analysis, (c) communication/collaborative skills. You will likely spend very little time teaching and most time either analyzing data or building software. What sort of mix are you looking for?  
    在高层次上，学术工作是研究和教学的结合。一个标准的以研究为导向的 “硬钱” 职位是支持你薪水的 75%（作为对你的教学和管理工作的回报），另外 25% 由补助金支付，你通常每年教授 2-3 门课程. “软钱”职位是指你的大部分（也许是全部）薪水来自赠款。在这个系统中，你通常每年会教授 0 门课程；也许你必须做一些客座演讲。我不太了解纯粹的教学职位，但我认为他们往往会得到 100% 的支持，但你每年教授 5-6 门以上的课程。我对行业工作知之甚少，但他们看重的技能往往是 (a) 软件工程，(b) 真正应用的分析，(c) 沟通 / 协作技能。您可能会花很少的时间在教学上，而大部分时间都花在分析数据或构建软件上。你在寻找什么样的组合？
*   If you are considering academics what kinds of students would you like to mentor?  
    如果你正在考虑学术，你想指导什么样的学生？
*   What kinds of colleagues would you like to have?  
    你喜欢什么样的同事？
*   Paste links for three jobs that you like. Why do you like those jobs?  
    粘贴您喜欢的三个工作的链接。你为什么喜欢这些工作？

_Who are you? 你是谁？_

*   What is the coherent “story of you” that you want your CV to tell?  
    您希望您的简历讲述的连贯的 “您的故事” 是什么？
...
*   What’s the 1-paragraph description of what you will do as an independent investigator? (Easy to find examples of such statements on websites of you favorite labs)  
    您作为独立调查员将做什么的 1 段描述是什么？ （在您最喜欢的实验室的网站上很容易找到此类声明的示例）
*   Who are the top 2 or 3 people/labs you admire and want to be like when you are independent  
    当你独立时，你最欣赏并希望成为的前 2 或 3 个人 / 实验室是谁

_How do you fit? 你怎么配？_
...
_How will you exhibit your mentoring and teaching abilities?  
你将如何展示你的指导和教学能力？_
...
*   Do you feel comfortable writing a blog or using social media professionally? (this is absolutely the best way to get an industry job as far as I can tell).  
    您是否觉得专业地写博客或使用社交媒体很自在？ （据我所知，这绝对是获得行业工作的最佳方式）。

_Recommendations 建议_
...
*   What have you learned / would you like to learn about the hiring process from the perspective of the people doing the hiring? For academics I can be a useful resource, for industry we can talk to lab/department alumni.  
    从招聘人员的角度，你学到了什么 / 你想了解招聘过程？对于学术界，我可以成为有用的资源，对于工业界，我们可以与实验室 / 部门的校友交谈。

_Where can you go for help?  
你可以去哪里寻求帮助？_

*   Who can you go to for questions about grants/scholarships/awards?  
    您可以向谁咨询有关助学金 / 奖学金 / 奖励的问题？
*   Do you know anyone who has applied for/got those grants/scholarships/awards?  
    您认识申请 / 获得这些助学金 / 奖学金 / 奖励的人吗？
*   Who can you go to for help about jobs (besides your advisor(s))?  
    您可以向谁寻求工作方面的帮助（除了您的顾问）？
*   Do you know anyone who had success on the market recently?  
    你认识最近在市场上取得成功的人吗？

_Note_: Part of this chapter appeared in the Simply Statistics blog post: [Science is a calling and a career, here is a career planning guide for students and postdocs](http://simplystatistics.org/2015/05/28/science-is-a-calling-and-a-career-here-is-a-career-planning-guide-for-students-and-postdocs/) and the [Leek group guide to career planning](https://github.com/jtleek/careerplanning) which was inspired by [Ben Langmead’s career planning guide](https://github.com/BenLangmead/langmead-lab/blob/master/postdoc_questionnaire.md).  
注意：本章的部分内容出现在 Simply Statistics 博客文章：Science is a calling and a career，这里有一份针对学生和博士后的职业规划指南和受 Ben Langmead 的职业规划指南启发的 Leek group 职业规划指南。

Your online identity 您的在线身份
---------------------------

### Your online identity - what should I do and why?  
您的在线身份 - 我应该做什么以及为什么？

**Have a standard handle 有一个标准的手柄**
...
在本书中，我们涵盖了一系列可用于执行研究、发布、交流和在线教学的工具。如果您使用这些工具来帮助您的学术生涯，您希望人们能够通过我们将使用的所有不同工具找到您。一种方法是识别一致的身份并在不同平台上使用它。

For example, whenever possible I use the handle _jtleek_. My Twitter, Github, and Gmail accounts all use that same handle. So when people are looking for me on a new platform, they know what my most likely username will be. This won’t always be possible, but when you can, choose a single username/handle and stick with it across platforms.  
例如，只要有可能，我都会使用句柄 jtleek。我的 Twitter、Github 和 Gmail 帐户都使用相同的句柄。因此，当人们在新平台上寻找我时，他们知道我最有可能的用户名是什么。这并非总是可行，但如果可以，请选择一个用户名 / 句柄并跨平台坚持使用。

**Use commercial platforms 使用商业平台**

In general you will be pushed toward using university email systems, data storage systems, and other infrastructure. Keep in mind that very rarely do academics stay at the same institution over time. For example, when you are a student, postdoc, or faculty member you will have an official university email account. This account will be useful for getting education discounts and will be the place that most official email will be sent. But you should, as early as possible, set up an email address on an independent platform like [Gmail](https://gmail.com/) that you won’t lose when you change jobs. When you publish papers, data, or code, you should use this email address as your address for correspondence.  
一般来说，你会被迫使用大学电子邮件系统、数据存储系统和其他基础设施。请记住，学者很少会长期留在同一个机构。例如，当您是学生、博士后或教职员工时，您将拥有一个官方大学电子邮件帐户。该帐户对于获得教育折扣很有用，并且是大多数官方电子邮件发送的地方。但是你应该尽早在 Gmail 这样的独立平台上建立一个你换工作时不会丢失的电子邮件地址。当您发表论文、数据或代码时，您应该使用此电子邮件地址作为您的通信地址。
...
请记住，如果您在互联网上进行科学研究，那么每个人都可以看到。这包括雇主、朋友、学生、拨款审查员、推荐信作者和您的妈妈。拥有与积极、建设性贡献相关的身份可能是一笔巨大的财富。拥有与混蛋相关的身份可能会给您带来现实生活中的问题。
...
使用商业电子邮件提供商而不是当地大学的电子邮件。商业电子邮件比您的大学帐户更有可能持续下去。拥有两封电子邮件也是明智的——一封用于个人通信，另一封用于您的职业角色。

*   [Gmail](https://gmail.com/) - the most widely used email address, also useful for a variety of other sites  
    Gmail - 使用最广泛的电子邮件地址，也可用于各种其他网站
*   [Yahoo](https://www.yahoo.com/) - still widely used, but sometimes perceived as a little past its prime.  
    雅虎 - 仍然被广泛使用，但有时被认为已经过了鼎盛时期。
*   [Outlook](http://outlook.com/) - recently making a comeback thanks to useful app designs, etc.  
    Outlook - 由于有用的应用程序设计等，最近卷土重来。

#### Data/file management 数据 / 文件管理
...
您可能会被要求使用您所在大学提供的数据管理系统。同样，最好使用商业提供商，这样您可以确保数据的持久性和控制。一个例外是个人身份或受监管的数据——这些数据必须小心控制，因此可能必须保留在大学系统中。

*   [Google Drive](http://drive.google.com/) - plenty of free storage space and plays nicely with Gmail  
    Google 云端硬盘 - 充足的免费存储空间，可与 Gmail 完美搭配
*   [Dropbox](http://www.dropbox.com/) - widely known and easy to use  
    Dropbox - 广为人知且易于使用
*   [Box](https://www.box.com/) - an alternative to Dropbox that is a little less widely known but easy to use  
    Box - Dropbox 的替代品，知名度较低但易于使用

### Your online identity - further tips and tricks  
您的在线身份 - 更多提示和技巧

#### Picking your username 选择您的用户名
...
关于用户名的一些建议是，它们应该 (a) 足够独特，以便它们可能跨平台使用，(b) 对您个人而言，但不要太可爱，以及 (c) 尽可能短，以便人们会能够很容易地记住它们。一个常见的选择是使用您的前两个首字母和您的姓氏，特别是如果您的姓氏很短。其他选项包括只使用您的名字和姓氏，中间用句点分隔，如果名字足够独特，则只使用您的名字，或者您不会不好意思告诉您的专业同事的昵称。
...
我是约翰霍普金斯大学彭博公共卫生学院的生物统计学副教授。我获得了博士学位。 2007 年在华盛顿大学获得生物统计学博士学位，然后在西奈山医学院干细胞生物学和约翰霍普金斯医学院计算生物学做短期博士后，然后加入约翰霍普金斯大学。

When I started my career I planned on doing the usual stuff - publishing papers in journals, teaching courses at Hopkins, and acting like a standard academic. Then in 2013, with my friends [Roger](https://twitter.com/rdpeng) and [Rafa](https://twitter.com/rafalab) I started a blog called [Simply Statistics](http://simplystatistics.org/). At first this was just a way for the three of us to share links, talk about frustrations with our profession, and point out cool things happening in the world of data science.  
当我开始我的职业生涯时，我计划做一些平常的事情——在期刊上发表论文，在霍普金斯大学教授课程，并表现得像一个标准的学者。然后在 2013 年，我与朋友 Roger 和 Rafa 一起开了一个名为 Simply Statistics 的博客。起初，这只是我们三个人共享链接、谈论我们职业的挫败感并指出数据科学领域发生的酷事的一种方式。

Over time I started to add new features to the blog, things like [interviews with junior data scientists](http://simplystatistics.org/interviews/) and [online conferences](http://simplystatistics.org/conferences/). As our readership grew, the blog ended up being a very powerful force in my career. People who didn’t know my papers or research very well still knew me through the blog. Our blog ended up growing a relatively large following on Twitter- [@simplystats](https://twitter.com/simplystats) - which then could be used to promote my scientific work, help my students get jobs, and in general increase my visibility in the scientific community.  
随着时间的推移，我开始向博客添加新功能，例如对初级数据科学家的采访和在线会议。随着读者人数的增加，博客最终成为我职业生涯中非常强大的力量。不太了解我的论文或研究的人仍然通过博客了解我。我们的博客最终在 Twitter 上获得了相对较多的关注 - [@simplystats](https://twitter.com/simplystats) - 然后可以用来宣传我的科学工作，帮助我的学生找到工作，并在总体上提高我在科学界的知名度。

Later, with two colleagues, I started to teach data science courses on Coursera. We now teach three large scale programs in [data science](https://www.coursera.org/specializations/jhu-data-science), [genomic data science](https://www.coursera.org/specializations/genomic-data-science), and [executive data science](https://www.coursera.org/specializations/executive-data-science) on that platform. When I started out teaching these courses, it wasn’t clear at all that there was going to be any benefit to my career to teaching online. But the large scale numbers in these courses have again given me an opportunity to raise my profile among an audience that didn’t previously know about me.  
后来，我和两个同事开始在 Coursera 上教授数据科学课程。我们现在在该平台上教授数据科学、基因组数据科学和执行数据科学的三个大型项目。当我开始教授这些课程时，完全不清楚在线教学对我的职业生涯有什么好处。但是这些课程中的大量学生再次让我有机会在以前不了解我的听众中提升自己的知名度。

### Should you follow my lead? 你应该听从我的领导吗？

People have often asked me whether teaching online, writing a blog, and spending time on Twitter are a good idea for junior scientists. The truth is I’m not entirely sure. There are definitely ways that being a modern scientist has benefited me. But I have had to spend a lot of energy on these endeavors and the payoff isn’t always entirely clear.  
人们经常问我在线教学、写博客和花时间在 Twitter 上对初级科学家来说是否是个好主意。事实是我不完全确定。作为一名现代科学家，肯定有很多方式让我受益。但我不得不在这些努力上花费大量精力，而回报并不总是很明显。

In general some of the best advice I’ve received during my academic career came from my PhD advisor [John](http://www.genomine.org/) and my first faculty mentor [Rafa](http://rafalab.dfci.harvard.edu/). They have both encouraged me to pursue things that I’m interested in, to try to show leadership in whatever things I’ve chosen to do, and to be aware of the important components of advancing my academic career, but not to be driven by them. So I’d pass that advice on.  
总的来说，我在学术生涯中收到的一些最佳建议来自我的博士生导师 John 和我的第一任教师导师 Rafa。他们都鼓励我追求我感兴趣的事情，尝试在我选择做的任何事情上表现出领导才能，并意识到推进我的学术生涯的重要组成部分，但不要被他们。所以我会把这个建议传递下去。