...
Physical Issues: Card Storage 物理问题：卡片存储
---------------------------------------
...
我尝试的第一种索引列出了 “重要” 卡片（我经常提到的卡片）。我现在只有其中之一。这个想法是，如果您发现很难找到一张卡片并希望它已列在您的索引中，您可以在该索引上写下一张卡片的名称和地址。这听起来应该比顶级编号卡片的简单列表更好，因为（正如我之前提到的）像 11a 这样的卡片通常比像 11 这样的卡片更重要。不幸的是，我发现这不是是这样的。问题是这种索引太难维护了。如果我一直在努力寻找一张卡片，我的工作记忆可能已经被找到那张卡片后我想做的所有事情负担过重。所以我忘记将它添加到索引中。
...
有时制作一张新的顶级卡片也很有意义，在上面列出与特定类别有关的所有内容。到目前为止，我只做过一次。这似乎是其他人使用的主要索引模式？但我不太喜欢这个主意。

_Listing Sibling Cards 列出兄弟卡_
...
当一张卡片有足够多的孩子难以追踪时，我会在所有其他孩子之前添加一张 “零” 卡片，这可以作为索引。因此，例如，卡片 2a 可能有孩子 2a1、2a2、2a3、... 2a15。这有很多需要跟踪的。所以我添加 2a0，它获得 2a1-2a15 的条目，以及在 2a 下添加的任何新卡。它还可以获得特别重要的后代的条目；也许 2a3a1 格外重要并获得一个条目。
...
对于像 2 这样的卡片，其孩子是按字母顺序排列的，你不能真正使用 “零” 来排在所有其他孩子之前。我使用 “λ” 作为 “字母零”——我将它排序，就好像它排在字母表中所有其他字母之前一样。因此，卡片“1λ” 列出了 1a、1b 等。
...
最重要的索引是 0 处的索引，ei，所有顶级编号卡的索引。正如我在 “卡片布局” 部分中所描述的，一张卡片大部分已经列出了它自己的孩子——这意味着你不需要添加一张新卡片来达到这个目的，直到事情变得笨拙为止。但是，顶级卡牌是没有父母可以追踪的！所以，您可能马上想要一张 “绝对零” 卡。
...
这些 “零” 卡还可以更轻松地跟踪是否已创建具有特定地址的卡。每做一张牌，就把它加到相应的零牌上；因此，您可以立即看到下一个可用地址。否则情况并非如此，特别是如果您的卡片当前未排序。

_Kimbro’s Mind MappingKimbro 的思维导图_
...
我已经尝试过改编 Lion Kimbro 的系统，该系统来自如何制作你所想的每一个想法的完整地图。毕竟，您认为每个想法的完整地图听起来都是完美的索引！
...
用我的术语来说，Lion Kimbro 只记笔记——他专注于收集和映射，而不是发展想法。笔迹被收集到主题和子主题中。当一个区域积累了足够的点数时，他就会开始为它制作思维导图。我不会详细介绍他所有的具体映射技巧（尽管它们是相关的），但基本上，想象一下将卡片的地址放入集群（在一张新的空白卡片上），然后写下描述这些集群的 “锚词”。

You built your tree in an initially “top-down” fashion, expanding trees by adding increasingly-nested cards. You’re going to build the map “bottom-up”: when a sub-tree you’re interested in feels too large to quickly grasp, start a map. Let’s say you’re mapping card 8b4. You might already have an index of children at 8b4λ; if that’s the case, you can start with that. Also look through all the descendants of 8b4 and pick out whichever seem most important. (If this is too hard, start by making maps for 8b4’s _children,_ and return to mapping 8b4 later.) Draw a new mind map, and place it at 8b4λa -- it is part of the index; you want to find it easily when looking at the index.  
您以最初 “自上而下” 的方式构建树，通过添加越来越多的嵌套卡片来扩展树。您将 “自下而上” 构建地图：当您觉得您感兴趣的子树太大而无法快速掌握时，请启动地图。假设您正在映射卡 8b4。您可能已经有了 8b4λ 处的儿童索引；如果是这样的话，你可以从那开始。还要查看 8b4 的所有后代并挑选出最重要的那个。 （如果这太难了，先为 8b4 的孩子做地图，然后再返回给 8b4 做地图。）画一个新的思维导图，把它放在 8b4λa——它是索引的一部分；您想在查看索引时轻松找到它。
...
现在，重要的是当你为 8b 制作地图时，你可以查看 8b4 的地图，以及 8b 的其他孩子拥有的任何地图。这意味着您不必遍历 8b 的所有后代（这很好，因为可能有很多）。您只需查看地图，它们已经为您提供了概览。 8b 的地图将从所有这些子地图中提取看似最重要的元素。
...
这允许重要的事情逐渐上升到顶部。当你在 0 处制作地图时，只需查看每个顶级编号卡片的地图，你就会从深层子树中获得所有最重要的东西。
...
从这样的映射中出现的类别可能与最初为您的顶级卡片播种的概念完全不同。如果需要，您可以制作与这些类别相对应的新顶级卡片。 （我没有这样做过。）
...
现在，当你在寻找东西时，你会从你的顶级地图开始。您查看集群并且可能对它的位置有一些期望（如果地址不在您的顶级地图上的某个地方）。你沿着地址找到更多的地图，这些地图会提供更多的地址群，直到你降落在一棵小到可以在没有地图的情况下导航的树上。
...
我已经将所有这些描述为一次性操作，但当然你会不断向这些地图添加内容，并在情况不再合适时重新绘制更新的地图。如果地图位于 8b40a，那么更新后的地图可以是 8b40b、8b40c 等等。您可以保留旧地图作为您不断变化的概念集群的历史记录。

Keeping Multiple Zettelkasten 保持多个 Zettelkasten
-----------------------------------------------
...
像 Zettelkasten（或 workflowy、dynalist、evernote 等）这样的笔记系统应该会陪伴你多年，与你一起成长并成为你想法的储存库。这是一个很大的承诺。
...
但是，如果您这样想，就很难优化笔记。如果你必须先三思而后行，你就无法进行实验。如果我认为我承诺要尝试将它作为我的 “下一个系统”，我将永远不会尝试 Zettelkasten——我认为它不会起作用。
...
同样，我无法以这种态度很好地优化我的 Zettelkasten。 Zettelkasten 应该是所有东西的一个存储库——例如，你不应该为一个新项目启动一个新的存储库。但是，我有几个 Zettelkasten，用于测试不同的格式：不同尺寸的卡片、不同的活页夹。仍然很难公平地对待替代品，因为我的两个主要 Zettelkasten 由于我在其中保留的内容而建立了势头。
...
我使用大写字母系统在我的 Zettelkasten 之间进行交叉引用。例如，我的主要 3x5 Zettelkasten 是 “S”（“小”）。我有另一个 Zettelkasten，它是 “M”，还有一个 “L”。在 S 中引用卡 1.1a 时，我将其称为 1.1a。如果我想从 M 中的一张卡片中引用它，我将其称为 S1.1a。等等。
...
显然卢曼做了类似的事情，开始了一个新的 Zettelkasten，偶尔会提到他的第一个。
...
但是，为特殊主题保留多个 Zettelkasten 不一定是个好主意。当心固定类别。危险在于类别会限制您编写的内容，或者随着时间的推移变得不那么合适。我过去曾尝试过专题笔记本，虽然它有时确实有效，但我常常会为把东西放在哪里而感到矛盾。 （诚​​然，关于在我的几个全主题 Zettelkasten 中放置东西的位置，我也有类似的冲突，但我在这里描述的大多数 3x5 系统已经胜出——目前。）
...
另一方面，我怀疑为 “非常不同” 的事物创建特殊主题 zettelkasten 是可以的。因为你正在写一本新书而创建一个新的 zettelkasten 可能是不好的——尽管它可以很好地实现组织写作材料的目标，但这意味着你下一本书的想法不是来自 Zettelkasten。 （Zettelkasten 应该首先包含 / 扩展产生书籍创意的思维过程，如果你必须有一个特定的书籍创意才能开始关于它的 zettelkasten，它就不能做得很好。）另一方面, 我怀疑为虚构的创意写作保留一个单独的 Zettelkasten 是可以的。事实的想法可以激发小说的想法，但是，这两者是完全不同的“模式”，因此将它们保存在物理上分开的集合中可能是有意义的。
...
使用扩展地址系统在多个 Zettelkasten 之间进行引用的想法也可以应用于解决 Zettelkasten 之外的其他事物。例如，您可能想想出一种将地址添加到旧笔记本的方法，以便您可以轻松地参考它们。 （例如，“笔记本编号：页码” 可能有效。）

Should You Transfer Old Notes Into Zettelkasten?  
您应该将旧票据转移到 Zettelkasten 吗？
-----------------------------------------------------------------------------
...
与此相关的是，由于 Zettelkasten 理想地成为了您一直在思考的所有事物的图书馆，因此尝试将现有笔记中的所有内容转移到 Zettelkasten 中可能很诱人。
...
（许多读者甚至可能不想这样做，因为这需要大量的工作。然而，那些对音符系统更认真的人可能认为这是一个好主意——或者，可能不敢尝试 Zettelkasten 因为他们认为他们必须这样做。）
...
我认为将旧的东西转移到 Zettelkasten 可能是有用的，但是，作为一个大项目试图让它立即发生很可能是不值得的。
...
*   It’s true that part of the usefulness of Zettelkasten is the interconnected web of ideas which builds up over time, and the “high-surface-area” format which makes it easy to branch off any part. However, not all the payoff is long-term: it should also be useful _in the moment._ You’re not _only_ writing notes because they may help you develop ideas in the future; the act of writing the notes should be helping you develop ideas now.  
    诚然，Zettelkasten 的部分用处是随着时间的推移建立起来的相互关联的思想网络，以及 “高表面积” 格式，可以很容易地从任何部分分支出来。然而，并非所有的回报都是长期的：它也应该在当下有用。你写笔记不仅仅是因为它们可以帮助你在未来发展想法；写笔记的行为现在应该可以帮助您发展想法。
*   You should probably only spend time putting ideas into Zettelkasten if you’re excited about further developing those ideas right now. You should not just be copying over ideas into Zettelkasten. You should be improving ideas, thinking about where to place them in your address hierarchy, interlinking them with other ideas in your Zettelkasten via address links, and taking notes on any new ideas sparked by this process. Trying to put all your old notes into Zettelkasten at once will likely make you feel hurried and unwilling to develop things further as you go. This will result in a pile of mediocre notes which will ultimately be less useful.  
    如果您现在对进一步发展这些想法感到兴奋，那么您可能只应该花时间将想法投入 Zettelkasten。你不应该只是将想法复制到 Zettelkasten 中。您应该改进想法，考虑将它们放在您的地址层次结构中的什么位置，通过地址链接将它们与 Zettelkasten 中的其他想法相互链接，并记下此过程引发的任何新想法。尝试一次将所有旧笔记放入 Zettelkasten 可能会让您感到匆忙，并且不愿意在进行时进一步开发。这将导致一堆平庸的笔记，最终将变得不那么有用。
*   I mentioned the breadth-first vs depth-first distinction earlier. Putting all of your old notes into Zettelkasten is an extremely breadth-first strategy, which likely doesn’t give you enough time to go deep into further developing any one idea.  
    我之前提到过广度优先与深度优先的区别。将你所有的旧笔记都放入 Zettelkasten 是一种极其广度优先的策略，这可能不会给你足够的时间来深入发展任何一个想法。
...
以一种漂亮的格式保存所有笔记的梦想怎么样？嗯，的确，不同格式的旧笔记可能更难找到，因为你必须记住你想要的笔记是用什么格式写的，或者检查你所有的旧笔记系统来找到你想要的笔记。不过，我认为解决这个问题不值得付出代价，尤其是因为您可能应该尝试许多不同的系统来找到适合您的好系统，而且您不能很好地将所有笔记移植到每个新系统系统。
...
与普通笔记本相比，Zettelkasten 应该是一个整体改进——如果不是，你就没有资格使用它。增加转移票据的巨额前期成本破坏了这一点。当您想使用 Zettelkasten 进一步发展想法时，只需选择 Zettelkasten。

Depth-first vs Breadth-first 深度优先与广度优先
--------------------------------------

Speaking of depth-first vs breadth-first, how should you balance those two modes?  
说到深度优先与广度优先，你应该如何平衡这两种模式？
...
幸运的是，这个问题背后有一些相关的计算机科学理论。我倾向于从迭代深化 A* 启发式搜索 (IDA*) 的角度来考虑它。
...
基本思想是这样的：深度优先搜索的优点是你可以通过只维护与你当前正在尝试的路径相关的信息来最小化内存成本。然而，深度优先搜索很容易陷入无结果的路径，而广度优先搜索有更好的保证。 IDA* 通过深度优先来平衡这两种方法，但当你太深时放弃，备份并尝试新路径。 （A* 方面是你定义 “太深” 的方式也取决于基于乐观评估的路径看起来有多有前途。）这样，你通过一系列深度优先模拟广度优先搜索冲刺。这使您可以一次将注意力集中在一小部分想法上。
...
一旦你探索了所有的路径达到一定的水平，你定义 “太深” 的容忍度就会增加，你就会重新开始。您可以将此视为越来越愿意花大量时间沿着困难的技术路径前进，因为您确认不存在更简单的选择。
...
当然，这不是您应该做什么的完美模型。但是，在我看来，笔记系统应该渴望支持和鼓励类似的东西。更一般地说，我想了解将您现有的研究方法视为一种算法（可能是一个糟糕的算法）的想法，并尝试思考如何改进它。不要试图强迫自己使用任何特定的算法，只是因为你认为你应该这样做；但是，如果你能找到方法让自己转向更有效的算法，那可能是个好主意。

Inventing Shorthand/Symbology 发明速记 / 符号
---------------------------------------
...
我不认为写作速度是思维速度的一大瓶颈。尽管我 “通过写作来思考”，但我的很多时间都花在了…… 嗯…… 思考上。然而，一旦我知道我想写什么，写作确实需要时间。当灵感真正袭来时，我可能会或多或少地知道我想在实际写到的地方之前的几段说什么。在那样的时候，似乎每一秒都很重要——我写得越快，我记下的想法就越多，我在开始之前忘记的就越少。
...
因此，似乎值得付出一些努力来加快写作速度。 （电脑打字显然也是这里要考虑的事情。）速记和特殊符号是可以尝试的。
...
还有空间问题。我知道我提倡使用小卡片，它们有意限制空间。但是，如果不需要，您不想浪费空间。关键是要尽可能容易地理解。编写要点并使用缩进来制作大纲是对传统段落的改进，因为它让您一目了然。同样，使用缩写和特殊符号将改善这一点。
...
我曾多次尝试学习 “正确的” 速记。也许我只是不够努力，但似乎基本上所有的速记系统都是通过遗漏信息来工作的。一旦你习惯了它们，在你写完它们后不久就可以很容易地阅读它们——当你或多或少还记得它们说的内容时。但是，如果您没有这样的猜测，它们实际上并没有传达足够的信息来完全恢复所写的内容。基本上，它们不会提高可读性。为了获得尽可能快的速度，他们将内容压缩到难以破译的程度。
...
另一方面，我花时间尝试改变我自己的笔迹，在不影响可读性的情况下提高速度。注意你最花时间写的东西，并想办法简化它。
...
Lion Kimbro 强调，您会想出一些方法来缩写您经常重复的事情。 Ho 描述了在他的系统中使用日语符号来表示一周中的几天和其他常见事物。 Bullet Journaling 社区创建了自己的符号系统。就个人而言，我已经尝试过各种不同的参考符号，它们表示不同种类的事物（除了我提到的 () 与 [] 的区别）。
...
Bullet Journaling 社区已经考虑了很多关于短而快的写作，目的是快速表达并在页面上留出更多空间。他们也有自己的符号系统，可能值得一看。 （我还没有使用它，但我最终可能会切换到它或类似的东西。）

Well, that’s all I want to say for now. I may add to this document in the future. For now, best of luck developing ideas!  
好吧，这就是我现在想说的。我可能会在将来添加到这个文档中。现在，祝你开发想法好运！
...
2020 年 6 月 12 日。我的索引卡 Zettelkasten 没有继续以最初的速度增长。但是，我仍然发现 Zettelkasten 的地址系统对于做笔记几乎是不可或缺的。 （我会说 “我无法想象我以前做过什么”，除了我确切地知道我以前做过什么；只是更糟。）我有几种不同格式的 Zettelkasten，我可以在它们之间切换。
...
我现在使用 Zettelkasten 的主要方式之一是在具有固定顺序页面的笔记本中。这意味着我永远无法对卡片进行排序；页面保持它们最初写入的顺序。然而，维护地址系统仍然非常有用，它让我有我本来无法拥有的想法。正如我所提到的，无论如何我都不经常对索引卡进行分类，而且我觉得这样做时我会丢失重要的新近信息。所以失去重新排列页面的能力并不是那么糟糕！虽然，有时我仍然会想念它，并且仍然使用其他格式来适当地重新排列页面。
...
我仍然主要发现 Zettelkasten 在几分钟、几周和几个月的顺序上很有用。在那之后，我的笔记开始变得 “​​陈旧”，我最好重新开始，而不是继续在我的旧笔记结构上构建一个特定的想法。 （旧笔记对于参考目的仍然有用，但我很少添加它们以进一步发展其中的想法，更愿意从新的想法树开始。）
...
这部分是因为 Zettelkasten 音符结构不是很容易重新排列，所以我无法 “修复” 旧结构以反映新思维。正如我所讨论的，数字笔记结构在很长一段时间内继续得到实际使用，这让我很幸运。但不仅如此——我仍然主要体验过数字格式的音符结构“变得陈旧”。
...
我看到对此有两种可能的回应。 (1) 尝试开发更好的技术来制作长期有用的结构。 (2) 顺势而为，为更直接的用处而优化。我倾向于（2）。我担心过度优化 (1) 只会以实际价值为代价创造假设价值——就像你无限期地延迟满足一样。
